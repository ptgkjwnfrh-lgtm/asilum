...
- **Shopify:** Admin API access exists only after a verified business completes
  OAuth. Credentials are encrypted, owner-scoped and revoked on disconnect or
  uninstall. Only products explicitly tagged `asilum:resale` publish.
...
- **Yahoo! Shopping Japan:** official API only, `used` + in-stock +
  cross-border filters, live response only; no storage/model use grant.
- **Rakuten Ichiba:** official API only and live response only. Ichiba is not a
  resale marketplace; only exact, reviewed secondhand shop codes may be
  labeled resale. All other results remain unknown and are excluded.
