...
| Wikipedia introductory text | one complete revision-pinned lead paragraph, shown as an excerpt | CC BY-SA 4.0 attribution/share-alike; article, revision/history and license links; record formatting cleanup | ACTIVE for verified designer/house records only | server-side MediaWiki query + exact-revision parse; no arbitrary HTML | withdraw/review on confirmed identity loss; preserve last-known-good on transient or suspicious refresh | 2026-09-19 |
| Wikimedia Commons images | individual cleared files only | file-specific creator, file page, license and modification record; never infer rights from a thumbnail | OPTIONAL; absent by default | only rights-verified image records can reach the public DTO | suppress image independently of licensed text | 2026-09-19 |
