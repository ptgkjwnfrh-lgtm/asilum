...
|---|---|---|---|---|---|---|
| eBay Browse API | inventory ingestion | official API, keys env-gated | **LIVE IN PRODUCTION** (12 Sep 2026 — real listings in `items` under `source_name='ebay'`). ⚖ The Prohibited AI Uses question in `epn-terms-check-2026-08-22.md` is NOT resolved; the owner ruled to proceed anyway (10 Sep: key the adapter; 12 Sep: "get real listings into items"), recorded in `ebay-catalog-ingest-2026-09-12.md`. Model exposure (clause 2) stays opt-in behind `--deep` | `EBAY_PARTNERSHIP_APPROVED` + keys; `DELETE FROM items WHERE source_name='ebay'` | counsel's five questions in the EPN brief | 2026-09-12 |
...
| Yahoo! JAPAN Auctions / Mercari / Rakuma (direct) | inventory ingestion | NONE. Mercari and Rakuma have no public API; Yahoo's is restricted | BLOCKED BY DESIGN — reached through a licensed proxy instead, never scraped | n/a (nothing built) | none — do not pursue direct scraping | 2026-08-27 |
