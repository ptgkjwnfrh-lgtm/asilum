...
| File | Lines | What it is |
| --- | ---: | --- |
| `evidence.js` | 153 | WHAT WE COULD SEE. Never a verdict. |
...
| File | Lines | What it is |
| --- | ---: | --- |
| `route.js` | 49 | GET /api/related?item=<id>&limit=<n> Pinterest-style "more like this": co-engagement graph neighbors first |
...
| File | Lines | What it is |
| --- | ---: | --- |
| `page.js` | 1094 | CATALOG (home). Straight clothing (owner order, Aug 12; POST folded into THE WIRE at /hotlist by the Aug 13 overhaul — all user posts live there now): |
...
| File | Lines | What it is |
| --- | ---: | --- |
| `page.js` | 572 | DISCOVER. The full site inventory across every source. Asterisk may route search and exploration through the user's Passport, but the user can pause that layer. |
| `layout.js` | 25 | Generated for route metadata only. The page itself is a client component and cannot export `metadata`, so the segment layout carries it. This renders its |
...
*Generated by `npm run docs:codemap` from main @ 23d1c7b — 321 source files, 59,425 lines. Do not edit this file by hand; edit `docs/code-map-preamble.md` or the source headers.*
