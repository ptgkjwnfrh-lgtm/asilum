...
test("Postgres adoption rekeys the identity_hash ledgers", { skip: !databaseUrl }, async (t) => {
  process.env.DATABASE_URL = databaseUrl;
  const db = await import("../lib/db/index.js?ledger-adoption=1");
  const production = await import("../lib/db/production.js?ledger-adoption=1");
  const pool = await db.getPool();
  const suffix = randomUUID();
...
test("Postgres adoption does not short-circuit past ledger-only signals",
  { skip: !databaseUrl }, async (t) => {
  process.env.DATABASE_URL = databaseUrl;
  const db = await import("../lib/db/index.js?ledger-shortcircuit=1");
  const production = await import("../lib/db/production.js?ledger-shortcircuit=1");
  const pool = await db.getPool();
  const suffix = randomUUID();
  const device = `u-${randomUUID()}`;
  const account = `sb-${randomUUID()}`;
