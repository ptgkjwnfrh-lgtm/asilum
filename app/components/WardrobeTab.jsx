...
async function prepPhoto(file) {
  ...
  small.getContext("2d").drawImage(bmp, 0, 0, 48, 48);
  if (bmp.close) bmp.close();
  const swatches = (dominantPalette(small.getContext("2d").getImageData(0, 0, 48, 48).data) || [])
...
export function WardrobeTab() {
  ...
  async function removePhoto(piece) {
    ...
    else setNotice("could not erase the photo");
  }
        ...
        <div className="wrow" key={piece.id}>
          ...
          {uploads.available && (
            <>
              ...
              <button
                className="wact"
                disabled={!photoConsent || busyPhoto === piece.id}
              ...
              {piece.photoPath ? (
                <button className="wact" onClick={() => removePhoto(piece)}>ERASE PHOTO</button>
              ) : null}
            </>
          )}
          {piece.status === "active"
