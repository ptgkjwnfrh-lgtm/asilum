...
export function assembleChunk(pool, taste, ctx = {}) {
  ...
  if (laneOn) {
    ...
    const lane = catalogLane(ordered, cursor, quotas.catalog, {
      exclude: (it) => pickedIds.has(it.id) || seen.has(it.id) || served.has(it.id) || !scoredById.has(it.id),
      defer: (it) => {
  ...
  if (picked.length < limit && !served.empty) {
  ...
  if (catalogOut) {
    catalogOut.count = c;
    // The cursor the client carries forward: the listing position and the
    // served filter with this page added.
    for (const it of picked) served.add(it.id);
    catalogOut.nextCursor = encodeCursor({ ...(laneNext || { end: true }), served });
  }
