...
export function blendedScore(item, taste, ctx = {}) {
  ...
  // (r18) gamma sparse-graph fallback: no behavioral edge → discounted
  // vector nearness to the recents stands in. Behavioral edges always win.
  ...
  const parts = {
    ...
    gamma: Math.max(behavioralGamma, vecNear * VEC_GAMMA_DISCOUNT),
...
export function assembleFeed(pool, taste, ctx = {}) {
  ...
  // POOL, so it must not be recomputed per item.
  ctx.noveltyIndex = ctx.noveltyIndex || buildNoveltyIndex(ctx.popularity);
