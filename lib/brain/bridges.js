...
export function blendedScore(item, taste, ctx = {}) {
  const split = activeSplit(!!ctx.epsilonActive);
...
export function assembleFeed(pool, taste, ctx = {}) {
  ...
  const scored = (pool || []).map((it) => {
    ...
    return {
      it, parts, direct,
      score: score * rot,
      discovery: vecSim(it.tags || {}, adjacent) * (0.35 + 0.65 * bridge) * rot,
  ...
  const reachN = hasTaste ? (ctx.epsilonActive ? REACH_BORED : REACH_BASE) : 0;
  ...
  const take = (s, zone) => {
    picked.push({ ...s.it, _score: s.score, _parts: s.parts, _zone: zone });
  ...
  while (picked.length < limit) {
    ...
    if (hasTaste && (slot + 1) % DISCOVERY_EVERY === 0) {
