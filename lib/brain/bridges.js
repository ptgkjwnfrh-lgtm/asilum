...
const SEEN_PENALTY = 0.3;     // score multiplier for the most recently served item...
const SEEN_PENALTY_OLDEST = 0.5; // ...grading up to this for the oldest in the ring — low enough that a
                                  // weakly-scored UNSEEN item still outranks a strongly-scored old one (#434)
...
export function assembleChunk(pool, taste, ctx = {}) {
  ...
  // below an unseen item's weight (SEEN_PENALTY_OLDEST < 1), so unseen items
  // always come first (#434: a heavy scroller reaches the whole pool) and only
  // AMONG seen items does the oldest-seen lead.
