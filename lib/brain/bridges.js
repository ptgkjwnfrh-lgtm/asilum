...
export function assembleChunk(pool, taste, ctx = {}) {
  ...
  const byScore = scored.slice().sort((a, b) => b.slate - a.slate);
  ...
  if (laneOn) {
    ...
    const cursor = decodeCursor(ctx.catalog && ctx.catalog.cursor);
    const lane = catalogLane(ordered, cursor, quotas.catalog, {
      exclude: (it) => pickedIds.has(it.id) || seen.has(it.id) || !scoredById.has(it.id),
  ...
  if (catalogOut) catalogOut.count = c;
