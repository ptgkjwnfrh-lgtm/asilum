 ...
 */
export function buildNoveltyIndex(popularity, populationIds = null) {
  if (!popularityDedupEnabled()) return null; // legacy path uses the raw curve
  // FOREIGN COUNTERS ARE EXCLUDED FROM THE RANKING POPULATION (21 Aug).
  // assembleFeed has always declared "a percentile is a property of the POOL"
  // — and then passed the whole popularity map, which counts entities no pool
  // can ever serve: wire transmissions (`txn-…`) carry counters in the same
  // table, and any probe row ever written lives there forever. Measured
  // against production that day: 53 foreign rows in 968 moved the percentile
  // of ALL 915 catalog items (mean 0.0068, max 0.0132) — small today, and
  // growing without bound as the wire fills.
  //
  // `populationIds` therefore FILTERS the counted rows; it does not replace
  // them. Ranking over the pool itself was tried first and is a different,
  // louder change: a pool item with no row would join the distribution as a
  // zero, and midrank ties then place every never-seen item at ~0.5 instead
  // of the ~1.0 the fallback gives it — epsilon's pull toward the unseen,
  // weakened in exactly the sparse worlds it exists for. tests/distribution
  // caught that within one run. Rows stay the evidence; the pool only says
  // which evidence is admissible.
  const admissible = populationIds ? new Set(populationIds) : null;
  const ids = Object.keys(popularity || {})
    .filter((id) => !admissible || admissible.has(id));
