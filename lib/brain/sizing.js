...
export function sizeRecord(rawLabel, opts = {}) {
  ...
  return {
    ...
    // the house, so a reader's own fit hint for it can be applied (fitHints.js)
    brand: opts.brand || null,
  };
}

// ---- The reader's own fit hints (lib/brain/fitHints.js keeps the pool-level
// helpers; these two live here so fitAssessment can read a hint without a
// circular import). Positive runsBias = runs small = fits like a SMALLER size.
const foldBrand = (s) => String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();

/** The net hint for one brand + category among a reader's hints, or null. */
export function fitHintFor(brand, category, hints = []) {
  if (!brand || !Array.isArray(hints) || !hints.length) return null;
  const want = foldBrand(brand);
  let small = 0, large = 0, matchedCategory = null;
  for (const h of hints) {
    if (!h || foldBrand(h.brand) !== want) continue;
    if (h.category && category && h.category !== category) continue;
    if (h.category && !category) continue;
    if (h.direction === "runs-small") small++;
    else if (h.direction === "runs-large") large++;
    else continue;
    if (h.category) matchedCategory = h.category;
  }
  if (small === large) return null;
  return { direction: small > large ? "runs-small" : "runs-large", steps: 1, brand, category: matchedCategory, votes: { small, large } };
}

/** A size record read through one hint: the label stays, the fit moves one step. */
export function applyFitHint(sizeRec, hint) {
  if (!sizeRec || typeof sizeRec !== "object" || !hint) return sizeRec;
  const idx = fitIndex(sizeRec.fitsLikeUS);
  const delta = hint.direction === "runs-small" ? -1 : 1;
  const next = idx == null ? sizeRec.fitsLikeUS : fitFromIndex(idx + delta * hint.steps);
  return {
    ...sizeRec,
    fitsLikeUS: next,
    runsBias: (Number(sizeRec.runsBias) || 0) - delta * hint.steps,
    hint: { direction: hint.direction, brand: hint.brand, category: hint.category, from: sizeRec.fitsLikeUS ?? null, to: next ?? null },
  };
}
...
export function fitAssessment(sizeRec, profile) {
  // A reader's own fit hint for this house is applied first (V.2): the ladder
  // reads the corrected size, the label is untouched, the hint is reported.
  if (profile?.fitHints?.length && sizeRec?.brand && !sizeRec.hint) {
    const hint = fitHintFor(sizeRec.brand, sizeRec.category, profile.fitHints);
    if (hint) {
      const read = applyFitHint(sizeRec, hint);
      return { ...fitAssessment(read, { ...profile, fitHints: [] }), hint: read.hint };
    }
  }
...
export function fitPhrase(sizeRec, profile) {
  // Read the record through the reader's hint once, then say what was done.
  let read = sizeRec;
  if (profile?.fitHints?.length && sizeRec?.brand && !sizeRec.hint) {
    const hint = fitHintFor(sizeRec.brand, sizeRec.category, profile.fitHints);
    if (hint) read = applyFitHint(sizeRec, hint);
  }
  const base = fitPhraseBase(read, profile);
  if (!base || !read?.hint) return base;
  return base + (read.hint.direction === "runs-small" ? " · read one size smaller for you" : " · read one size larger for you");
}

function fitPhraseBase(sizeRec, profile) {
