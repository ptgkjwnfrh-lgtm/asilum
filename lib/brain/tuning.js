...
// dwell; skip negative). Only attributed events count. Keys come from the
// frozen EVENTS vocabulary — hand-written string literals silently drop the
// whole action (a wrong name is indistinguishable from an unweighted event).
const EVENT_ACTION_WEIGHT = {
  // a purchase outranks a bag: settled by the processor, or reported by the reader
  [EVENTS.USER_COMPLETED_PURCHASE]: 3, [EVENTS.USER_REPORTED_PURCHASE]: 2.5,
  [EVENTS.USER_ADDED_TO_BAG]: 2, [EVENTS.USER_SHARED_ITEM]: 1.5,
  [EVENTS.USER_ADDED_TO_MOOD_BOARD]: 1.3, [EVENTS.USER_SAVED_ITEM]: 1.2,
  [EVENTS.USER_FAVORITED_ITEM]: 1, [EVENTS.USER_VIEWED_PRODUCT]: 0.3,
  [EVENTS.USER_SKIPPED_PRODUCT]: -1, [EVENTS.USER_REJECTED_RECOMMENDATION]: -1,
};

/** The event types that carry tuning weight — asserted against EVENTS in tests. */
export function weightedEventTypes() {
  return Object.keys(EVENT_ACTION_WEIGHT);
}

/** Feature flag: the bridges may self-tune their weights within bounds. ON unless `BRAIN_BRIDGE_TUNING=0` — a missing
