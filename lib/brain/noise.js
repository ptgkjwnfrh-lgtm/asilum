...
/**
 * The p-th percentile of `xs`, p in 0..1, by nearest-rank.
 *
 * Nearest-rank rather than interpolated on purpose: these samples are
 * measurement outputs, and an interpolated percentile reports a value that no
 * run actually produced. Returns 0 for an empty sample — callers treat "no
 * data" and "zero" alike here, since a battery with no runs has no floor.
 */
