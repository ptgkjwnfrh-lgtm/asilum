 ...
 */
export function simulateSession(bot, policy, { pool, pages = 6, limit = 60 } = {}) {
  ...
  for (let page = 0; page < pages; page++) {
    ...
    const { items } = buildFeed({ ...stateSnapshot, splitOverride: policy || undefined, limit }, pool);
    const interactions = [];
    ...
    log.push({ stateSnapshot, interactions });
  }
  ...
  return { botId: bot.id, log, satisfaction, positives, fastSkips, viewed };
}
