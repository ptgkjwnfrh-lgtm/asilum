...
export function simulateSession(bot, policy, { pool, pages = 6, limit = 60, tune = false } = {}) {
  ...
  const edges = {};
  const popularity = {};
  // (Aug 6) The counters count PEOPLE. One bot is one identity, so seeing the
  // same item on twelve pages must leave viewers at 1 — otherwise this
  // simulator, a third implementation of the counting rule, would keep
  // measuring the pre-fix behaviour and every battery in the round would pass
  ...
  for (let page = 0; page < pages; page++) {
    ...
    for (let slot = 0; slot < Math.min(SLOTS_VIEWED, items.length); slot++) {
      const it = items[slot];
      popularity[it.id] = popularity[it.id] || { eng: 0, imp: 0, engagers: 0, viewers: 0 };
      popularity[it.id].imp += 1;              // diagnostic: raw exposures
      if (!seenOnce.has(it.id)) { seenOnce.add(it.id); popularity[it.id].viewers += 1; }
      viewed++;
      ...
      if (verdict.action === "bag" || verdict.action === "favorite") {
        ...
        popularity[it.id].eng += 1;            // diagnostic: raw engagements
        if (!engagedOnce.has(it.id)) { engagedOnce.add(it.id); popularity[it.id].engagers += 1; }
  ...
  // popularity is returned so a battery can assert the counting RULE the
  // simulator implements — an unasserted third implementation is how a round
  // measures its own pre-fix behaviour and passes.
  return { botId: bot.id, log, satisfaction, positives, fastSkips, viewed, bridgeImpressions: botImp, popularity };
}
