...
/**
 * Build `n` synthetic readers for the offline replay harness.
 *
 * SEEDED AND DETERMINISTIC — the same seed yields the same population, which
 * is what makes a replay result comparable across runs. Any real randomness
 * here would turn every measurement into a fresh experiment.
 *
 * Each bot gets a two-tag latent taste and judges pieces by affinity-expanded
 * taste rather than its literal tags, so the harness measures whether the feed
 * found something the reader would LIKE, not whether it matched a string.
 */
