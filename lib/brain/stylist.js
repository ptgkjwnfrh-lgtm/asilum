...
// won't fit.
//
// "conf" is a calibrated *display* of relative score (75–99 — see the clamp
// below; this line said 58–99 until Aug 17 and had been wrong since the floor
// moved). It is a ranking signal that sharpens as the profile accumulates
// interactions — not a literal purchase probability, and NOT a percentage.
// Keep it honest in the UI copy: /stylist prints it as "MATCH n of 99", with
// no % sign, because a % reads as one.
//
// WHAT THIS MEANS FOR THE FLOOR, and it is an open owner ruling (8), not a
// defect to silently repair: `raw` below cannot be negative, so conf is
// structurally >= 75 and the "match floor 75" enforced in /api/outfits can
// never reject a look. Every look ships, and the worst possible one still
// displays 75. Narrowing the display range would start rejecting looks — a
// product decision. See docs/metric-definitions-2026-08-17.md §M3.
...
export function buildOutfit(pool, taste = {}, opts = {}) {
  ...
  // cohAvg >= 0 and the taste term is floored at 0, so raw is in [0, 1] — which
  // makes `Math.max(75, …)` unreachable and the product's "match floor 75" a
  // filter that never fires. Left exactly as it ships: changing the mapping is
  // owner ruling 8, not a cleanup. Do not "simplify" the max away either — it
  // is what pins the documented floor to this line.
