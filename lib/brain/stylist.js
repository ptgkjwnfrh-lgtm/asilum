...
import { sizeWithHints } from "./fitHints.js";
...
function candidateScore(item, taste, chosen, fitProfile) {
  ...
  if (fitProfile && item.size) {
    const fs = fitScore(sizeWithHints(item, fitProfile?.fitHints), fitProfile); // sizing brain
...
export function buildOutfit(pool, taste = {}, opts = {}) {
  ...
  for (const slot of wantSlots) {
    ...
    if (fitProfile && best.c.size) best.s.phrase = fitPhrase(sizeWithHints(best.c, fitProfile?.fitHints), fitProfile);
