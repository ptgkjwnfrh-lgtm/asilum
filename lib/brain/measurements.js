...
export function measurementProfileForDisplay(storage = {}) {
  ...
  for (const key of MEASUREMENT_KEYS) {
    const raw = inches[key];
    // Number(null) === 0 and Number("") === 0: both are ABSENCE, not zero.
    // A stored NULL column reaches here as null (pg) or undefined (mem).
    if (raw == null || raw === "") { profile[key] = ""; continue; }
