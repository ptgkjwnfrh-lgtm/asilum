...
export function buildFeed(state = {}, pool = CATALOG) {
  ...
  const { items, catalog, quotas } = assembleChunk(pool, taste, {
    ...
    // (6 Sep, late) how the taste lanes rotate between chunks — "memory" when
    // the caller persists served ids (the seen penalty rotates), "cursor"
    // when it does not (offsets ride the cursor). freshTaste restarts the
    // taste lanes at the top of a new ranking (a re-chunk after actions).
    rotation: state.rotation === "memory" ? "memory" : "cursor",
    freshTaste: !!state.freshTaste,
  });
...
export function recordServe(profile, serveId, items = []) {
  ...
  const entry = { id: String(serveId), bridges, slots, zones, reported: false };
  ...
  // card outside the latest chunk lost its slot context. A short ring keeps
  // the last SERVE_RING serves; lastServe stays the newest for older readers.
  p._meta.serves = [entry, ...(Array.isArray(p._meta.serves) ? p._meta.serves : []).filter((s) => s && s.id !== entry.id)]
    .slice(0, SERVE_RING);
...
export function applyExaminationReport(profile, serveId, counts = {}) {
  ...
  const id = String(serveId);
  const done = { ...serve, reported: true };
