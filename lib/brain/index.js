...
// The dominant tag is one of the TEN aesthetics. Descriptors (lowercase keys,
// the stream's deeper array) ride in the same object and can carry a higher
// weight than any aesthetic, but a streak, a skip penalty or the beta bridge
// is reasoned in aesthetics — a descriptor never becomes "dominant".
function dominantTag(item) {
  const t = item && item.tags ? item.tags : {};
  let best = null, bestV = 0;
  for (const k in t) {
    if (!TAGS.includes(k)) continue;
    if (t[k] > bestV) { bestV = t[k]; best = k; }
  }
  return best;
}

// A profile may carry at most this many descriptor keys; the weakest go
// first. The 256 KiB profile budget is the reason; descriptors are the
// stream, not the whole memory.
export const DESCRIPTOR_KEYS_CAP = 400;
function capDescriptorKeys(vec) {
  const keys = Object.keys(vec).filter((k) => !TAGS.includes(k));
  if (keys.length <= DESCRIPTOR_KEYS_CAP) return;
  keys.sort((a, b) => Math.abs(vec[a]) - Math.abs(vec[b]));
  for (const k of keys.slice(0, keys.length - DESCRIPTOR_KEYS_CAP)) delete vec[k];
}
...
export function buildFeed(state = {}, pool = CATALOG) {
  ...
  const { items, catalog, quotas } = assembleChunk(pool, taste, {
    ...
    // the stream's prior per item (lib/db/production/stream.js) — bends alpha
    streamPrior: state.streamPrior || null,
