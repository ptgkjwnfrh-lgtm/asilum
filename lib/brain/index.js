...
// Engagement hierarchy: purchase intent beats sharing beats saving beats liking.
const ACTION_WEIGHT = {
  bag: 0.7,        // added to bag / purchased (incl. imported buyer history)
  share: 0.6,
  save: 0.5,
  favorite: 0.4,
...
export function buildFeed(state = {}, pool = CATALOG) {
  ...
  return { split, items, epsilonActive, epsilonAuto, zones };
}
...
export function relatedItems(sourceItem, { edges, pool = CATALOG, limit = 8 } = {}) {
  ...
  for (const it of catalogByAffinity(sourceItem.tags || {}, limit * 2)) {
