...
export function recordServe(profile, serveId, items = []) {
  ...
  const entry = { id: String(serveId), bridges, slots, zones, reported: false, examined: [] };
  ...
  // older readers.
  const ring = [entry, ...(Array.isArray(p._meta.serves) ? p._meta.serves : []).filter((s) => s && s.id !== entry.id)];
  p._meta.serves = trimServeRing(ring);
...
export function trimServeRing(ring) {
  ...
  for (const s of ring) {
    if (!s) continue;
    if (out.length && (out.length >= SERVE_RING_MAX || cards + serveCards(s) > SERVE_RING_CARDS)) break;
    out.push(s);
    cards += serveCards(s);
  }
  while (out.length > 1 && JSON.stringify(out).length > SERVE_RING_BYTES) out.pop();
  return out;
}
