...
import { CHUNK_MIN } from "../feed/rechunk.js";
export { CHUNK_MIN };
...
// positive is skipped by the taste lanes for the session; the rotating fill
// pass (cursor field `f`) reaches it once the pool is dry, so it is late,
// never lost. Past ~3000 held ids the filter saturates — the rendered
// ceiling is 300 and a reload starts a fresh one.
...
function fnv1a(str, seed) {
  ...
  // Finalizer: SERVED_BITS is a power of two, so positions use the low bits
  // only, and FNV's low bits evolve independently of the high ones — two
  // sequential ids differing in two characters could cancel exactly and
  // share all six positions (a verifier measured 206 such pairs in 20,000
  // `syn-NNNN` ids; 6 expected). Mixing the high bits down removes it.
...
export function encodeCursor(pos) {
  ...
  // `f`: where the dry-pool fill pass resumes, so a dry pool rotates through
  // the whole ranking instead of repeating one page — and reaches an item
  // the filter wrongly calls served, which no other lane can.
  const f = Number.isFinite(pos.fill) && pos.fill > 0 ? { f: Math.trunc(pos.fill) } : {};
  if (pos.end) return b64(JSON.stringify({ v: 1, end: true, ...s, ...f }));
  return b64(JSON.stringify({ v: 1, t: Number(pos.t) || 0, id: String(pos.id), ...s, ...f }));
}
...
export function decodeCursor(raw) {
  ...
  try {
    ...
    const fill = Number.isFinite(Number(obj.f)) && Number(obj.f) > 0 ? Math.trunc(Number(obj.f)) : 0;
    if (obj.end === true) return { end: true, served, fill };
    if (typeof obj.id !== "string" || !obj.id) return null;
    const t = Number(obj.t);
    return { t: Number.isFinite(t) ? t : 0, id: obj.id, served, fill };
  } catch {
    return null;
  }
}
