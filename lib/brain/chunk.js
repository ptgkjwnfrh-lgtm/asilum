...
export const CURSOR_MAX_LEN = 1200;
...
export function encodeCursor(pos) {
  if (!pos) return null;
  if (pos.end) return b64(JSON.stringify({ v: 1, end: true }));
 ...
 *
 * Returns { items, nextCursor, exhausted }.
 */
export function catalogLane(ordered, cursor, n, { exclude = () => false, defer = () => false } = {}) {
  ...
  if (firstDeferred) {
    return {
      items,
      nextCursor: encodeCursor({ ...listingKey(firstDeferred), skip: takenAfterDeferral }),
      exhausted: false,
    };
  }
  ...
  return {
    items,
    nextCursor: resumeAt ? encodeCursor(listingKey(resumeAt)) : encodeCursor({ end: true }),
    exhausted: !resumeAt,
  };
}
