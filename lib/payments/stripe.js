...
// One session per order. amountCents is trusted server state (never a client
// figure); origin is the caller's canonical origin — routes pass SITE_ORIGIN,
// local verification passes its own localhost.
export async function createCheckoutSession({ orderId, itemId, title, amountCents, currency, origin, user }) {
