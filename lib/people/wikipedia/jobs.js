...
import {
  ...
  fetchWikipediaOverview, resolveNameCandidate, retryAtFor,
} from "./client.js";
import { discoverySource, WIKIPEDIA_DISCOVERY_SOURCES } from "./registry.js";
import { coverageRegionForCountry } from "./regions.js";
...
async function discoverCandidates(payload) {
  ...
  if (!source?.enabled || !["wikidata", "wikipedia_category", "wikipedia_list"].includes(source.type)) {
  ...
  const found = source.type === "wikidata"
    ...
    : source.type === "wikipedia_list" ? await discoverWikipediaList({
      title: source.title, language: source.language || "en", kind, limit,
...
export async function queueWikipediaDiscoveryCycle({ now = new Date(), limit = 20 } = {}) {
  const sources = WIKIPEDIA_DISCOVERY_SOURCES.filter((source) =>
    source.enabled && ["wikidata", "wikipedia_category", "wikipedia_list"].includes(source.type));
  ...
  const offset = source.type === "wikipedia_list"
    ? (Math.floor(day / sources.length) % 80) * Math.max(1, Math.min(25, Number.parseInt(limit, 10) || 20))
    : 0;
  return [await enqueueWikipediaJob({
    jobKey: `discover:${source.key}:${window}`,
    kind: "discover",
    payload: { sourceKey: source.key, limit: Math.max(1, Math.min(25, Number.parseInt(limit, 10) || 20)), offset },
  })];
}
