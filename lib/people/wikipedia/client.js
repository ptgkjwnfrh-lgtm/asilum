...
const RELEVANCE = Object.freeze({
  en: {
    designer: /\b(fashion designer|clothing designer|menswear designer|womenswear designer|apparel designer|shoe designer|footwear designer|fashion stylist|couturier|designer\s+(?:best\s+)?known\s+for\s+(?:his|her|their)\s+fashion|designer of (?:clothing|fashion|menswear|womenswear|ready-to-wear))\b/i,
    house: /\b(fashion(?: and (?:lifestyle|accessories?))? (?:house|brand|label|company|business)|(?:high )?fashion (?:women[’']?s |men[’']?s )?clothing and accessor(?:y|ies) brand|clothing(?: and accessories)? (?:brand|company|label)|apparel (?:brand|company|label)|menswear (?:brand|label|house)|womenswear (?:brand|label|house)|luxury (?:fashion|goods) (?:house|brand|company)|leather goods (?:house|brand|company|maker)|ready-to-wear (?:brand|label|house))\b/i,
  },
  fr: {
...
/** Read the Wikipedia-maintained nationality list as candidate discovery.
 * Linked articles still pass through the same QID, kind and lead relevance
 * checks before publication; list membership never becomes overview prose. */
export async function discoverWikipediaList({
  title, language = "en", kind = "designer", limit = 1000, offset = 0,
}, { fetchImpl = fetch } = {}) {
  if (!["designer", "house"].includes(kind)) throw new WikimediaError("kind_invalid", "invalid fashion entity kind");
  const cap = Math.max(1, Math.min(5000, Number.parseInt(limit, 10) || 1000));
  const start = Math.max(0, Number.parseInt(offset, 10) || 0);
  const url = new URL(apiUrl(language));
  for (const [key, value] of Object.entries({
    action: "parse", format: "json", formatversion: "2", page: title,
    prop: "text|revid", disableeditsection: "1", disablelimitreport: "1",
  })) url.searchParams.set(key, value);
  const body = await wikimediaFetchJson(url, { fetchImpl });
  const { document } = parseHTML(String(body?.parse?.text || ""));
  const root = document.querySelector(".mw-parser-output");
  if (!root) throw new WikimediaError("list_unusable", "Wikipedia designer list has no parser output");
  const candidates = [];
  const seen = new Set();
  let country = null;
  const ignored = new Set(["See also", "References", "External links", "Notes"]);
  for (const node of root.children) {
    const heading = node.tagName === "H2" ? node
      : node.classList?.contains("mw-heading2") ? node.querySelector("h2") : null;
    if (heading) {
      country = String(heading.textContent || "").replace(/\[edit\]$/i, "").trim();
      if (ignored.has(country)) country = null;
      continue;
    }
    if (!country) continue;
    for (const anchor of node.querySelectorAll("li > a:first-child")) {
      const href = String(anchor.getAttribute("href") || "");
      const articleTitle = String(anchor.getAttribute("title") || "").trim();
      const canonicalName = String(anchor.textContent || "").trim();
      if (!href.startsWith("/wiki/") || href.includes("#") || !articleTitle || articleTitle.includes(":")) continue;
      const key = `${kind}:${articleTitle}`;
      if (seen.has(key)) continue;
      seen.add(key);
      candidates.push({
        canonicalName, wikipediaTitle: articleTitle, language, kind, country,
        listRevisionId: Number(body.parse.revid) || null,
      });
    }
  }
  return candidates.slice(start, start + cap);
}
