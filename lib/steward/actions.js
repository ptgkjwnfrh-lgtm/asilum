...
//
// Every SQL statement here re-checks the state the PLAN selected on — not
// just the status column but the condition itself (`and moderation_status =
// 'visible' and (price is null …)`, `and status = 'observed' and
// distinct_identities >= …`, `and not exists (select 1 from items …)`), so a
// row a person changed between plan and act is not touched — it is counted
// as the world having moved, and reported. MEASURED (the 6 Sep verification
// of the handover): two of the eight statements re-checked less than the plan
// had selected on, and a row a person had fixed in between would have been
// touched with a matching count. tests/steward-hands.test.js now asserts the
// predicates by text, because the fake world honours any predicate.
//
    ...
    const { rows } = await q(
      `delete from edges e
        ...
        where e.a = x.a and e.b = x.b
          and (not exists (select 1 from items i where i.id = e.a)
            or not exists (select 1 from items i where i.id = e.b))
        returning e.a`,
