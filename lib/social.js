...
// backend exists), follow state, profile info, tracking mocks, and the
// on-device observation toggle. Pure JS — safe to import from server routes
// (window access is always guarded).

import { hashStr } from "./client.js";

// ---- Marketplace sources -----------------------------------------------------
// Simulated listings get a deterministic source label so the marketplace layer
// (Discover filters, stylist source counts, orders) behaves like the real
// cross-site product. Real ingested items keep their true source.

export const SOURCES = ["ASILUM Vault", "SSENSE", "Farfetch", "Lowheads", "eBay", "Grailed", "Depop"];

export function sourceFor(item) {
  if (!item || !item.id) return SOURCES[0];
  if (item.id.startsWith("ebay-") || item.source === "eBay Browse API") return "eBay";
  return SOURCES[hashStr(String(item.id)) % SOURCES.length];
}

// ---- Mock users (placeholder data until real accounts exist) ------------------
...
export function listPosts() {
  ...
  return [...mine, ...SEED_POSTS].sort((a, b) => b.at - a.at);
}
...
// ---- Ticket / tracking mocks (until carrier APIs are wired) --------------------

const CARRIERS = ["DHL", "UPS", "FedEx", "USPS"];

export function trackingFor(orderId, at) {
  const h = hashStr(String(orderId) + ":" + String(at || 0));
  const age = Date.now() - (at || Date.now());
  const day = 1000 * 60 * 60 * 24;
  const status = age < day ? "label created" : age < 4 * day ? "in transit" : "delivered";
  const eta = new Date((at || Date.now()) + 5 * day);
  return {
    ticket: "ASM-" + (h % 1e8).toString(36).toUpperCase().padStart(6, "0"),
    carrier: CARRIERS[h % CARRIERS.length],
    status,
    eta: status === "delivered" ? "arrived" : eta.toLocaleDateString(undefined, { month: "short", day: "numeric" }),
  };
}
