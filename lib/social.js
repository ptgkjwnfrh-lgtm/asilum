...
// A GHOST is a real listing Asterisk read, shown for demonstration while the
// app is in demo: real photographs, a real link to the source, and no
// purchase ticket — it may sell at the source any minute. Not a demo record
// (the photographs and the reading are real) and not live inventory.
export function isGhostItem(item) {
  return !!item && item.listing_kind === "ghost";
}
export const GHOST_LABEL = "GHOST";
export const GHOST_NOTE =
  "ghost listing — a real piece read by Asterisk, shown for demonstration. it lives at its source and cannot be bought here.";
/**
 * Does this page actually have a demo record on it?
 *
 * THE DISCLOSURE FOLLOWS THE DATA, IT DOES NOT ASSERT IT. The page-level
 * banners were written on 16 August, when every row in the catalog was
 * synthetic, and they stated it unconditionally. On 12 September the seed was
 * deleted and 897 real eBay listings took its place — and the same sentences
 * went on telling every visitor that real, linkable, purchasable pieces were
 * "synthetic sample data with placeholder imagery". A false disclosure in that
 * direction is worse than none: it reads as honesty while it lies, and four
 * tests held it green because they asserted the SENTENCE rather than the fact.
 *
 * So the banner asks the shelf. An owned wardrobe piece is not catalog and is
 * skipped; an empty page makes no claim at all.
 */
export function anyDemoRecord(items = []) {
  return (Array.isArray(items) ? items : []).some((item) => item && !item.owned && isDemoItem(item));
}
