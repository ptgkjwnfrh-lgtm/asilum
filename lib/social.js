...
export async function fetchWire(kind = "user", limit = 60) {
  ...
  const isMatch = (local, srv) =>
    norm(local.text) === norm(srv.text) &&
  ...
  try {
    ...
    for (const s of server) if (mine.some((m) => isMatch(m, s))) s.mine = true;
    const posts = [
      ...server,
      ...mine.filter((m) => !server.some((s) => isMatch(m, s))),
    ].sort((a, b) => b.at - a.at);
