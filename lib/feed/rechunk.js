...
// The rendered ceiling of the catalog page — one quantity, one name; the
// serve ring is bounded by it too.
export const MAX_RENDERED = 300;
// The smallest and default chunk the server serves; the client asks for
// CHUNK after the first load. Declared here (client-safe) and imported by
// lib/brain/chunk.js, so the two ends cannot drift.
...
export const SERVE_RING_CARDS = MAX_RENDERED;
export const SERVE_RING_MAX = 16;
export const RECHUNK_MIN_REPLACE = 12; // fewer replaceable cards → leave it to the next scroll (= the smallest chunk the server serves)
 ...
 */
export function placeInColumns(items, count, memo, weight = null) {
  ...
  const w = (it) => (typeof weight === "function" ? Number(weight(it)) || 1 : 1);
  (items || []).forEach((it, idx) => {
    if (!it || it.id == null) return;
    const id = String(it.id);
    // An id twice in the list is one card: the first occurrence is the card.
    if (placed.has(id)) return;
    live.add(id);
    const c = map.get(id);
    if (c !== undefined && c < n) { cols[c].push({ it, idx }); heights[c] += w(it); placed.add(id); }
  });
  ...
  (items || []).forEach((it, idx) => {
    ...
    const id = String(it.id);
    if (placed.has(id)) return;
    // The shortest column by HEIGHT when the caller can say how tall a card
    // is (the tile aspect), else by count — CSS multicol balanced by height,
    // and a count-balanced grid's tail was ragged by up to two screens.
    let best = 0;
    for (let c = 1; c < n; c++) if (heights[c] < heights[best]) best = c;
    map.set(id, best);
