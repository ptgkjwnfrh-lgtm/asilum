...
export const FACETS = Object.freeze({
  ...
  // the stream's two (schema v51, lib/asterisk/stream): what a collector
  // names when they look closely, and what the piece evokes
  detail: {
    means: "a construction or hardware fact — dagger collar, metal zippers, wool lining",
    weight: 0.9, aliases: ["construction", "hardware"], written: true,
  },
  reference: {
    means: "a cultural reference the piece earns by its look and period — a film, a record, a scene, a person",
    weight: 0.8, aliases: ["cultural-reference", "movie", "film", "music"], written: true,
  },
...
// NULL-PROTOTYPE ON PURPOSE. A facet name arrives as a STRING from a caller —
// `addProductTags` takes one straight off an admin request body — and a plain
// object answers for names nobody put in it: `facetOf("constructor")` returned
// `Object`, which is TRUTHY, so the one guard that makes this register a fence
// ("a facet nobody defined cannot be written") let it through, and the write
// died three layers down on the v49 CHECK naming a constraint instead of a
// cause. `__proto__` did the same. A lookup keyed by untrusted strings has to
// be a map that only holds what was put in it, not an object with ancestors.
const ALIAS_TO_FACET = Object.freeze(Object.assign(Object.create(null),
  Object.fromEntries(
...
// Null-prototype for the same reason: `weighTypedTagScores` does
// `TYPE_WEIGHTS[tagType] ?? 0.3` over facet names read back out of the table,
// and an inherited hit there multiplies a score by a FUNCTION — NaN, which is
// not > 0, so the piece silently leaves the results rather than scoring at the
// unknown floor the `?? 0.3` was written to give it.
export const FACET_WEIGHTS = Object.freeze(Object.assign(Object.create(null),
  Object.fromEntries(
