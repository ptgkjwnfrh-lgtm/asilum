...
export function createGlobe(canvas, { tokens, reducedMotion = false, onFront = null } = {}) {
  ...
  const draw = () => {
    ...
    // the cage: far side first, dim; near side over it, bright. NO shadow
    // per glyph (9 Sep): a canvas shadowBlur on each of ~700 glyphs cost
    // /upload 37ms a frame at idle against 16.7 elsewhere — the owner's
    // Safari lagged — so the near side's light is alpha alone, and the halo
    // is one soft pass of the whole cage drawn once at low alpha beneath.
    const pts = [];
    for (const p of cage) {
    ...
    for (const [px, py, z, g, w] of pts) {
      const near = z >= 0;
      const depth = near ? 0.55 + z * 0.45 : 0.18 + (z + 1) * 0.08;
