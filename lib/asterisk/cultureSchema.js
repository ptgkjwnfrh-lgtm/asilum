...
export function normalizeCultureProposal(input = {}, { allowedKinds, takenNames }) {
  ...
  for (const raw of input.interpretations) {
    ...
    // AN OUTFIT BREAKDOWN (owner, 10 Sep): the pieces a look is made of, each
    // with the descriptors the stream uses (lib/asterisk/stream) — "dagger
    // collar", "rust satin lining" — so a reference and a listing meet on
    // the same words. Optional; at most 8 pieces, 8 descriptors each.
    let pieces = [];
    if (raw.pieces !== undefined) {
      if (!Array.isArray(raw.pieces) || raw.pieces.length > 8) return { ok: false, error: `${id}: pieces must be an array of at most 8` };
      for (const rawPiece of raw.pieces) {
        if (!rawPiece || typeof rawPiece !== "object") return { ok: false, error: `${id}: each piece must be an object` };
        const piece = clean(rawPiece.piece, 60).toLowerCase();
        if (!piece) return { ok: false, error: `${id}: a piece needs a name` };
        const descriptors = normalizeTerms(rawPiece.descriptors, { field: `${id}: ${piece} descriptors`, maxItems: 8, maxLength: 30 });
        if (descriptors.error) return { ok: false, error: descriptors.error };
        const year = rawPiece.year === undefined ? null : Number(rawPiece.year);
        if (year !== null && !(Number.isInteger(year) && year >= 1900 && year <= 2030)) return { ok: false, error: `${id}: ${piece} year out of range` };
        pieces.push({ piece, descriptors: descriptors.terms, ...(year ? { year } : {}) });
      }
    }
