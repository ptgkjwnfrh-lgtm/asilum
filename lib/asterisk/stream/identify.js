...
import { IdentificationSchema, capIdentification, DESCRIPTOR_KINDS } from "./schema.js";
// THE SHAPE IS ASKED FOR IN THE PROMPT AND ENFORCED LOCALLY. The API's
// structured-output grammar refused this record ("The compiled grammar is
// too large", 10 Sep 2026 — fourteen capped lists, a ten-key object, an
// enum), so the model is told the exact JSON shape, the answer is parsed
// and validated by the same zod schema, and one corrective turn is spent
// when it does not validate. (This also retires the SDK zod helper import
// that trap 164 — the repo's leading "*" — made awkward locally.)
...
ANSWER WITH ONE JSON OBJECT AND NOTHING ELSE — no prose before or after, no code fence — in exactly this shape (types in angle brackets; use null where a field is unknown; every list may be empty):
{
 "identification": {"house": <string|null>, "line": <string|null>, "designer": <string|null>, "garment": <string>, "collection": <string|null>, "season": <string|null>,
   "year": {"value": <integer|null>, "low": <integer|null>, "high": <integer|null>, "confidence": <0..1>}, "origin": <string|null>, "what_it_is": <one sentence>},
 "evidence": [<string>, … up to 8],
 "parent_tag_readings": [{"parent": "<Name=Value as given>", "meaning": <string>}, … up to 24],
 "construction": [<string>, …], "materials": [<string>, …], "fit": [<string>, …], "details": [<string>, …],
 "references": [<string>, …], "subcultures": [<string>, …], "mood": [<string>, …],
 "descriptors": [{"tag": <lowercase string, at most 30 characters>, "weight": <0..1>, "kind": <one of ${DESCRIPTOR_KINDS.map((k) => `"${k}"`).join(" | ")}>}, … up to 40],
 "taste": {${TAGS.map((t) => `"${t}": <0..1>`).join(", ")}},
 "confidence": <0..1>
}`;
...
// The JSON object in a text block: the outermost braces, a code fence
// tolerated. Returns { data } or { error } — never throws.
export function extractRecord(text) {
  if (!text) return { error: "empty" };
  const start = text.indexOf("{"), end = text.lastIndexOf("}");
  if (start < 0 || end <= start) return { error: "no json object" };
  let parsed;
  try { parsed = JSON.parse(text.slice(start, end + 1)); } catch (e) { return { error: `unparseable: ${String(e.message).slice(0, 120)}` }; }
  const out = IdentificationSchema.safeParse(parsed);
  if (!out.success) {
    const issues = out.error.issues.slice(0, 6).map((i) => `${i.path.join(".")}: ${i.message}`).join("; ");
    return { error: `invalid: ${issues}` };
  }
...
export async function identifyListing(ctx, { client = null, config = identifyConfig() } = {}) {
  ...
  let message = null, iterations = 0, corrections = 0, searched = false, lastError = null;
  for (; iterations < MAX_ITERATIONS; iterations++) {
    ...
    const got = extractRecord(lastText(message));
    if (got.data) {
      return {
        model: config.model, promptVersion: PROMPT_VERSION, usage, costUsd: costOf(config.model, usage), searched,
        iterations: iterations + 1, corrections, latencyMs: Date.now() - started,
        record: capIdentification(got.data), status: "ok",
      };
    }
    lastError = got.error;
    if (corrections >= MAX_CORRECTIONS) break;
    corrections += 1;
    messages.push({ role: "assistant", content: message.content });
    messages.push({ role: "user", content: `That answer did not validate (${got.error}). Return ONLY the JSON object in the exact shape given, nothing else.` });
  }
  const base = { model: config.model, promptVersion: PROMPT_VERSION, usage, costUsd: costOf(config.model, usage), searched, iterations: iterations + 1, corrections, latencyMs: Date.now() - started, record: null };
  if (message?.stop_reason === "refusal") return { ...base, status: "refused" };
  return { ...base, status: lastError ? (lastError.startsWith("invalid") ? "invalid" : "unparseable") : "empty", error: lastError };
}
