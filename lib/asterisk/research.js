 ...
 *
 * `selfFactId` (compile-time re-validation only): the loaded catalog contains
 * PREVIOUSLY COMPILED research records, so a fact re-validated on the next
 * compile pass would otherwise collide with its own earlier output and the
 * fail-closed compile would refuse every batch after the first. A collision
 * with a record carrying this fact's own id is self-recognition, not a
 * conflict; every other collision still refuses.
 */
export function validateCultureProposal(input = {}, { selfFactId = null } = {}) {
  const index = cultureIndex();
  // Self-recognition (compile idempotency): a collision with the record this
  // fact itself compiled into earlier is not a conflict; anything else is.
  const takenNames = selfFactId
    ? { has: (key) => { const hit = index.get(key); return !!hit && hit.factId !== selfFactId; } }
    : index;
  return normalizeCultureProposal(input, {
    allowedKinds: new Set(allowedCultureKinds()), takenNames,
  });
}
...
/** The culture proposals awaiting review. Filtered to proposal claims only —
 *  the learned-facts table holds other kinds, and a review queue that mixed
 *  them would invite approving something reviewed under different rules. */
...
export async function approvedCultureProposals() {
  ...
  for (const fact of approved) {
    ...
    // selfFactId: a fact must not collide with its OWN previously compiled
    // record on the next compile pass (see validateCultureProposal).
    const validated = validateCultureProposal(parsed, { selfFactId: fact.id });
