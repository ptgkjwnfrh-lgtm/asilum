...
export function validateCultureProposal(input = {}) {
  ...
  for (const raw of interps) {
    ...
    if (!/^[a-z0-9][a-z0-9-]{0,39}$/.test(id.slice(name.length + 1))) {
      return { ok: false, error: `interpretation id "${id}" has a malformed slug` };
    }
    ...
    if (new Set(tags).size !== tags.length) return { ok: false, error: `${id}: tags must be unique` };
    for (const t of tags) {
