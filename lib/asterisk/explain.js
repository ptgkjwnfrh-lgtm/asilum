...
export async function recordCorrection(userId, { productId = null, code, note = "", scope = "ongoing", serveId = null } = {}) {
  ...
  try {
    ...
    const { correction, moderationTask, duplicate, revived } = await createUserCorrectionWithEvent(
      ...
      {
        userId, type: EVENTS.USER_CORRECTED_RECOMMENDATION,
        payload: { productId, code, lane, scope, brand: item?.brand || null, tags, serveId: serveId || null, policyVersion: POLICY_VERSION },
      },
      report
    );
