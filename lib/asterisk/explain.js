...
// A piece that is not in the database is not in the catalog — so Asterisk does
// not explain it, and does not accept a correction about it. The file is
// consulted only where there is no database to ask (memory mode), which is the
// same rule lib/products.js getDiscoverablePool applies. Before 12 September
// 2026 this was `|| getCatalogItem(id)` unconditionally, which meant that after
// the seed was deleted /api/why went on producing full, confident, honest-
// looking readings of 915 pieces that no longer existed.
function catalogFloor(productId) {
  return process.env.DATABASE_URL ? null : getCatalogItem(productId);
}
...
export async function explainProduct(userId, productId) {
  try {
    const item = await getItem(productId) || catalogFloor(productId);
