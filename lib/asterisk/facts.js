...
export async function recordFact(input = {}) {
  ...
  const sourceUrls = (Array.isArray(input.sourceUrls) ? input.sourceUrls : [])
    .map(String).filter((u) => /^https?:\/\//.test(u)).slice(0, 10);
  ...
  if (input.payload !== undefined) {
    ...
    if (json.length > 8192) return { ok: false, error: "payload exceeds 8 KiB" };
    value = json;
  }
  const fact = await createLearnedFact({
    ...
    value,
    sourceUrls,
    sourceTypes: (Array.isArray(input.sourceTypes) ? input.sourceTypes : []).map(String).slice(0, 10),
    publicationDates: (Array.isArray(input.publicationDates) ? input.publicationDates : []).map(String).slice(0, 10),
...
export async function reviewFact(id, nextStatus, reviewedBy = null) {
  ...
  const updated = await updateLearnedFactStatus(id, nextStatus, reviewedBy);
