...
import { laneOf, priceCeilingFrom, fitHintsFrom } from "./correctionSignals.js";
import { POLICY_VERSION } from "../brain/policy.js";
import { getBoards } from "../db/index.js";
import { resolveProducts } from "../products.js";
import { findCareerEntity, careerOf, getCareerEntity } from "../people/careers.js";
...
/**
 * EVERYTHING ASTERISK REMEMBERS ABOUT ONE PERSON, assembled for them to read.
 *
 * The contract behind ADR-001 and the /asterisk surface: a reader can see what
 * is held about them, where it came from, and hide sections they do not want
 * shown. Reads are gathered in parallel and the result is a REPORT, not a
 * store — nothing here mutates.
 *
 * Returns `{ ok: false }` rather than throwing for a missing user, so the
 * surface can say so plainly.
 */
export async function asteriskMemory(userId) {
  if (!userId) return { ok: false, error: "user required" };
  const [corrections, follows, measurements, styleProfile, brainProfile, learned, preferences, boards] =
    await Promise.all([
  ...
  const designers = await creditedDesigners(boards);
  ...
  return {
    ...
    memory: {
      ...
      explicit: {
        // WHAT APPLIES NOW (synergy round): the lanes a reader's corrections
        // currently hold open — the price ceiling recommendation surfaces
        // honour and the fit hints the ladder reads. Undone and expired
        // session corrections are absent here exactly as they are there.
        lanes: {
          priceCeilingCents: priceCeilingFrom(corrections),
          fitHints: fitHintsFrom(corrections),
        },
        corrections: corrections.map((c) => ({
          id: c.id, code: c.code, lane: laneOf(c.code), scope: c.scope || "ongoing",
          undone: !!(c.undoneAt ?? c.undone_at),
      ...
      inferred: {
        // THE PEOPLE BEHIND THE SAVES (19 Sep): the designers a reader's saved
        // pieces credit, counted from the catalog's stored credits and tied to
        // the career registry — never inferred from a house name alone.
        designers,
...
/**
 * The designers a reader's saved pieces credit, most-saved first, with the
 * registry id and the houses of their sourced career when the registry knows
 * them. A house written into designers[] ("Gucci" on a Gucci piece) is a
 * house, not a person, and is skipped.
 */
export async function creditedDesigners(boards = [], { limit = 5 } = {}) {
  const ids = [...new Set((boards || []).flatMap((b) => (b.items || []).map((it) => it && it.id).filter(Boolean)))];
  if (!ids.length) return [];
  const products = await resolveProducts(ids).catch(() => new Map());
  const counts = new Map();
  for (const it of products.values()) {
    for (const name of it.designers || []) {
      if (!name || name === it.brand) continue;
      const cur = counts.get(name) || { name, saves: 0, houses: new Set() };
      cur.saves++; if (it.brand) cur.houses.add(it.brand);
      counts.set(name, cur);
    }
  }
  return [...counts.values()]
    ...
    .slice(0, limit)
    .map((d) => {
      const entity = findCareerEntity(d.name, "designer");
      const career = entity ? careerOf(entity.id).map((e) => ({ house: getCareerEntity(e.houseId)?.name || e.houseId, role: e.role, start: e.start, end: e.end })) : [];
      return { name: d.name, id: entity ? entity.id : null, saves: d.saves, savedAt: [...d.houses], career, sourced: !!entity };
    });
}
