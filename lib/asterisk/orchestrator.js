...
/** A raw query reduced to the canonical form every layer below matches
 *  against — lowercased, markup characters dropped, whitespace collapsed,
 *  length-capped. One normalization, so two layers cannot disagree about what
 *  the reader typed. */
...
/**
 * Promote the reading that matches the reader's leftover words — "70s Bowie"
 * surfaces the seventies reading of Bowie rather than the strongest one.
 *
 * ORDERING ONLY. READINGS ARE NEVER ERASED (the queryRouter brief): with no
 * leftover-token match, the existing taste and confidence order stands
 * untouched. Disable with SEARCH_ERA_TARGETING=0, no deploy needed.
 */
