...
export function validateCompiledResearchRecord(rec, { kinds, taken }) {
  if (!rec || typeof rec !== "object" || Array.isArray(rec)) return "record must be an object";
  if (!kinds.has(rec.kind)) return `unsupported kind: ${rec.kind}`;
  if (typeof rec.name !== "string" || !/^[a-z0-9][a-z0-9 '&.-]{1,59}$/.test(rec.name)) return "malformed name";
  const names = [rec.name, ...(Array.isArray(rec.aliases) ? rec.aliases : [])];
  if (new Set(names).size !== names.length) return "duplicate name or alias";
  if (names.some((name) => typeof name !== "string" || !/^[a-z0-9][a-z0-9 '&.-]{1,59}$/.test(name))) return "malformed alias";
  if (names.some((name) => taken.has(name))) return "name or alias collides with the catalog";
  if (!Array.isArray(rec.interpretations) || rec.interpretations.length < 1 || rec.interpretations.length > 6) {
    return "1-6 interpretations required";
  }
  const ids = new Set();
  for (const interpretation of rec.interpretations) {
    if (!interpretation || typeof interpretation !== "object") return "malformed interpretation";
    if (typeof interpretation.id !== "string" || !interpretation.id.startsWith(rec.name + "/") ||
        !/^[a-z0-9][a-z0-9-]{0,39}$/.test(interpretation.id.slice(rec.name.length + 1))) return "malformed interpretation id";
    if (ids.has(interpretation.id)) return "duplicate interpretation id";
    ids.add(interpretation.id);
    if (typeof interpretation.label !== "string" || !interpretation.label.trim() ||
        typeof interpretation.summary !== "string" || !interpretation.summary.trim()) return "interpretation label and summary required";
    if (!Array.isArray(interpretation.tags) || interpretation.tags.length < 1 || interpretation.tags.length > 5 ||
        new Set(interpretation.tags).size !== interpretation.tags.length ||
        interpretation.tags.some((tag) => typeof tag !== "string" || !TAGS.includes(tag.toUpperCase()))) return "invalid interpretation tags";
    if (typeof interpretation.confidence !== "number" || interpretation.confidence <= 0 || interpretation.confidence > 0.8) {
      return "invalid interpretation confidence";
    }
  }
  if (typeof rec.factId !== "string" || !rec.factId.startsWith("fact-")) return "missing fact provenance";
  if (typeof rec.provenance !== "string" || !rec.provenance.startsWith("research-approved-")) return "missing review provenance";
  if (!Array.isArray(rec.sourceUrls) || !rec.sourceUrls.length || rec.sourceUrls.some((url) => !safeExternalUrl(url))) {
...

{
  ...
  for (const rec of RESEARCH) {
    const error = validateCompiledResearchRecord(rec, { kinds, taken });
