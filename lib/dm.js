...
export function describeRefusal(code, { callerBlockedThem = false, knockPending = false } = {}) {
  ...
  switch (code) {
    ...
    // 42501 reaches this from the reaction path two ways: a mark that is not
    // in the palette (the law table refuses it), and a reaction from someone
    // who is not in the conversation — which no surface can produce. One
    // sentence covers both without claiming the person is unreachable.
    case "42501":
