...
export async function createOrderWithEvent({ user, itemId, amountCents, currency }) {
  const order = {
    id: newOrderId(),
    user_id: user,
    item_id: itemId,
  ...
  try {
    ...
    await client.query(
      `INSERT INTO orders (id, user_id, item_id, status, amount_cents, currency)
       VALUES ($1,$2,$3,'created',$4,$5)`,
      [order.id, user, itemId, amountCents, currency]
    );
    await client.query(
