...
export async function createOrderWithEvent({ user, itemId, amountCents, feeCents = 0, currency }) {
  const order = {
    id: newOrderId(),
    user_id: user,
    item_id: itemId,
  ...
  try {
    ...
    await client.query(
      `INSERT INTO orders (id, user_id, item_id, status, amount_cents, fee_cents, currency)
       VALUES ($1,$2,$3,'created',$4,$5,$6)`,
      [order.id, user, itemId, amountCents, feeCents, currency]
    );
    await client.query(
