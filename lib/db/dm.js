...
export async function exportMessagesFor(meRaw, caps = {}, { queryTarget = null } = {}) {
  ...
  return {
    ...
    truncated: {
      ...
      messages: messages.rows.length >= messageCap || convos.rows.length >= conversationCap,
    ...
    oldestExportedMessageId: messages.rows.length >= messageCap && messages.rows.length
