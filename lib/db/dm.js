 ...
 */
export async function exportMessagesFor(meRaw, caps = {}) {
  const me = accountId(meRaw);
  const conversationCap = Math.max(1, Math.trunc(Number(caps.conversations)) || 500);
  const messageCap = Math.max(1, Math.trunc(Number(caps.messages)) || 5000);
  const blockCap = Math.max(1, Math.trunc(Number(caps.blocks)) || 500);
  const p = await pool();
  ...
  const blocks = await p.query(
    `SELECT blocked_account_id, source, created_at
       FROM dm_blocks WHERE blocker_account_id = $1
      ORDER BY created_at DESC LIMIT $2`, [me, blockCap]);
