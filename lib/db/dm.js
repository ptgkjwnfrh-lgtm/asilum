...
export async function exportMessagesFor(meRaw, caps = {}, { queryTarget = null } = {}) {
  ...
  return {
    ...
    settings: settings.rows[0]
      ...
      : null,
    caps: { conversations: conversationCap, messages: messageCap, blocks: blockCap },
    // WHERE THE CUT FELL. The message cap is global and ordered newest-first,
    // so hitting it does not thin every conversation evenly — it removes the
    // oldest messages, and with them the whole content of the oldest
    // conversations. `bounded()` says THAT it truncated; this says WHERE, so a
    // reader can tell which end of their history they are missing rather than
    // being left to guess.
    oldestExportedMessageId: messages.rows.length
 ...
 *
 * Returns { typing, readUpTo }, both null whenever this reader may not see
 * them: their signals off, YOUR signals off, a block either way, an unaccepted
 * knock, or a conversation you are not in. The route does not forward
 * `reciprocal` — that field is this function's own account of which case it
 * was, useful to the tests and to a server-side caller, and a payload that
 * names the reason defeats the very indistinguishability the nulls provide.
 *
 * WHAT THIS DOES NOT CLAIM, because the old docstring claimed it while the
 * payload contradicted it in the same breath: a reader whose own signals are
 * on, in an accepted unblocked thread they are in, can infer from a null that
 * the other side's signals are off. That is not hideable without inventing a
 * read position, and inventing one is a lie about a person's attention. The
 * setting is a courtesy both sides can see the shape of; it is not a secret,
 * and the honest thing is to say so rather than to describe a property the
 * code cannot deliver.
 */
