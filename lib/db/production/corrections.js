...
import { SEEN_CAP } from "../../brain/index.js";
...
const CORRECTION_SCOPES = ["item", "session", "ongoing"];
// A session-scoped correction speaks for 24 hours (lib/asterisk/correctionSignals.js).
const SESSION_SCOPE_SQL = "(undone_at IS NULL AND (scope <> 'session' OR created_at > now() - interval '24 hours'))";
const memActive = (c, now = Date.now()) =>
  !c.undoneAt && ((c.scope || "ongoing") !== "session" || now - c.createdAt < 24 * 60 * 60 * 1000);
...
function correctionDbRow(r) {
  return {
    ...
    scope: r.scope || "ongoing",
    undoneAt: r.undone_at ? new Date(r.undone_at).getTime() : null,
...
export async function createUserCorrectionWithEvent(c, evt, moderation = null) {
  const row = {
    ...
    scope: CORRECTION_SCOPES.includes(c.scope) ? c.scope : "ongoing",
  ...
  if (!p) {
    ...
    if (existing && !existing.undoneAt) return { correction: existing, moderationTask: null, duplicate: true };
    if (existing) {
      // REVIVE: the same correction after an undo is a new act, not a replay.
      existing.undoneAt = null; existing.scope = row.scope; existing.tags = row.tags; existing.createdAt = Date.now();
      memRecordEvent(n);
      return { correction: existing, moderationTask: null, duplicate: false, revived: true };
    }
    ...
    const correction = push(mem.corrections,
      { id: mem.seq++, ...row, undoneAt: null, createdAt: Date.now(), persistent: false });
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO user_corrections (user_id, product_id, brand, code, tags, note, scope)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT (user_id, product_id, code) WHERE product_id IS NOT NULL DO NOTHING
       RETURNING id, user_id, product_id, brand, code, tags, note, scope, undone_at, created_at`,
      [row.userId, row.productId, row.brand, row.code, JSON.stringify(row.tags), row.note, row.scope]
    );
    if (!rows[0]) {
      const existing = await client.query(
        `SELECT id, user_id, product_id, brand, code, tags, note, scope, undone_at, created_at
         FROM user_corrections WHERE user_id=$1 AND product_id=$2 AND code=$3`,
        [row.userId, row.productId, row.code]);
      if (existing.rows[0]?.undone_at) {
        // REVIVE: the same correction after an undo is a new act, not a replay.
        const revived = await client.query(
          `UPDATE user_corrections SET undone_at=NULL, scope=$4, tags=$5, created_at=now()
           WHERE user_id=$1 AND product_id=$2 AND code=$3
           RETURNING id, user_id, product_id, brand, code, tags, note, scope, undone_at, created_at`,
          [row.userId, row.productId, row.code, row.scope, JSON.stringify(row.tags)]);
        await client.query(EVENT_INSERT_SQL, [n.userId, n.type, n.payloadJson, n.at]);
        await client.query("COMMIT");
        return { correction: correctionDbRow(revived.rows[0]), moderationTask: null, duplicate: false, revived: true };
      }
      await client.query("COMMIT");
...
export async function getUserCorrectionSignalSummary(userId) {
  ...
  if (!p) {
    const relevant = mem.corrections.filter((correction) =>
      correction.userId === userId && CORRECTION_SIGNAL_CODES.includes(correction.code) && memActive(correction));
  ...
  const { rows } = await p.query(
    `WITH corrections AS MATERIALIZED (
       ...
       WHERE user_id=$1 AND code=ANY($2::text[]) AND ${SESSION_SCOPE_SQL}
     ), signals AS (
...
// The codes whose PRODUCT is excluded from recommendation: the reader owns
// it, it is not their style, they are not interested — and a lane code whose
// scope is "this item" (too expensive / too small / too large for THIS piece).
const ITEM_EXCLUSION_CODES = ["already-own", "not-my-style", "not-interested"];
const ITEM_SCOPED_LANE_CODES = ["too-expensive", "too-small", "too-large", "dislike-finish"];

function priceCeilingOf(rows, now = Date.now()) {
  let ceiling = null;
  for (const c of rows) {
    if (c.code !== "too-expensive" || (c.scope || "ongoing") === "item") continue;
    const tag = (c.tags || []).map(String).find((t) => t.startsWith("price:"));
    const cents = Number(tag ? tag.slice(6) : NaN);
    if (!Number.isFinite(cents) || cents <= 0) continue;
    ceiling = ceiling == null ? cents : Math.min(ceiling, cents);
  }
  return ceiling;
}
function fitHintsOf(rows) {
  const tagOf = (c, p) => (c.tags || []).map(String).find((t) => t.startsWith(p + ":"))?.slice(p.length + 1) ?? null;
  return rows.filter((c) => c.code === "too-small" || c.code === "too-large").map((c) => ({
    direction: c.code === "too-small" ? "runs-small" : "runs-large",
    brand: tagOf(c, "brand") || c.brand || null, category: tagOf(c, "category"),
    size: tagOf(c, "size"), productId: c.productId, scope: c.scope || "ongoing",
  }));
}

/**
 * What recommendation surfaces must honour: excluded brands and products,
 * the price ceiling (the lowest price called too expensive, minor units, or
 * null) and fit hints. Undone and expired-session rows have no effect.
 */
export async function getUserRecommendationExclusions(userId) {
  const p = await getPool();
  if (!p) {
    const corrections = mem.corrections.filter((correction) => correction.userId === userId && memActive(correction));
    return {
      ...
      productIds: [...new Set(corrections
        .filter((correction) => ITEM_EXCLUSION_CODES.includes(correction.code) ||
          (ITEM_SCOPED_LANE_CODES.includes(correction.code) && (correction.scope || "ongoing") === "item"))
        .map((correction) => correction.productId).filter(Boolean))],
      priceCeilingCents: priceCeilingOf(corrections),
      fitHints: fitHintsOf(corrections),
    };
  }
  const { rows } = await p.query(
    `SELECT
       ...
       COALESCE(array_agg(DISTINCT product_id) FILTER (
         WHERE product_id IS NOT NULL AND (code = ANY($2::text[]) OR (code = ANY($3::text[]) AND scope='item'))), '{}') AS product_ids
     FROM user_corrections WHERE user_id=$1 AND ${SESSION_SCOPE_SQL}`,
    [userId, ITEM_EXCLUSION_CODES, ITEM_SCOPED_LANE_CODES]);
  const lanes = await p.query(
    `SELECT product_id, brand, code, tags, scope FROM user_corrections
     WHERE user_id=$1 AND code IN ('too-expensive','too-small','too-large') AND ${SESSION_SCOPE_SQL}`,
    [userId]);
  const laneRows = lanes.rows.map((r) => ({ productId: r.product_id, brand: r.brand, code: r.code, tags: r.tags || [], scope: r.scope }));
  return {
    brands: rows[0]?.brands || [], productIds: rows[0]?.product_ids || [],
    priceCeilingCents: priceCeilingOf(laneRows), fitHints: fitHintsOf(laneRows),
  };
}

/**
 * Undo one correction: stamp undone_at, keep the row, write the event in the
 * same transaction. `{ correction }` or `{ correction: null }` when nothing
 * active matched (already undone, someone else's, or unknown).
 */
export async function undoUserCorrection(userId, correctionId, evt) {
  const n = normalizeEvent(evt);
  if (n.userId !== userId) throw new TypeError("undo and event user must match");
  const p = await getPool();
  if (!p) {
    const row = mem.corrections.find((c) => c.userId === userId && String(c.id) === String(correctionId) && !c.undoneAt);
  ...
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
      `UPDATE user_corrections SET undone_at=now()
       WHERE user_id=$1 AND id=$2::bigint AND undone_at IS NULL
       RETURNING id, user_id, product_id, brand, code, tags, note, scope, undone_at, created_at`,
      [userId, /^\d+$/.test(String(correctionId)) ? String(correctionId) : "-1"]);
    if (!rows[0]) { await client.query("ROLLBACK"); return { correction: null }; }
    await client.query(EVENT_INSERT_SQL, [n.userId, n.type, n.payloadJson, n.at]);
    await client.query("COMMIT");
    return { correction: correctionDbRow(rows[0]) };
  } catch (e) {
    await client.query("ROLLBACK").catch(() => {});
    throw e;
  } finally {
...
export async function getUserCorrectionState(userId, { productId = null, brand = null } = {}) {
  ...
  if (!p) {
    return {
      alreadyOwn: !!productId && mem.corrections.some((c) =>
        c.userId === userId && c.productId === productId && c.code === "already-own" && !c.undoneAt),
      excludedBrand: !!normalizedBrand && mem.corrections.some((c) =>
        c.userId === userId && c.code === "dont-recommend-brand" && !c.undoneAt &&
  ...
  const { rows } = await p.query(
    `SELECT
     ...
     WHERE user_id=$1 AND undone_at IS NULL AND ((product_id=$2 AND code='already-own') OR
...
function mergeTasteProfiles(targetRaw, sourceRaw, { targetMass = 1, sourceMass = 1 } = {}) {
  ...
  const merged = {
    ...
    _meta: {
      ...
      // RECENT_CAP 20 (lib/brain/index.js), seen → SEEN_CAP (IMPORTED, see
      // below), activity → ACTIVITY_MAX 30 (lib/brain/memory.js), looksSeen →
      // SEEN_LOOKS_CAP 300 (app/api/outfits), follows → FOLLOW_CAP 10
      // (app/api/follow). A larger merge cap let the next writer silently
      // truncate — or, for follows, never (it only slices on add), leaving the
      // feed blending 100 boards.
      //
      // `seen` is imported rather than copied because the copy DID go stale:
      // it said 200 while the writer moved to a pool-sized ring (6 Sep), so
      // signing in would have thrown away most of a reader's rotation memory
      // and put them back inside the ceiling the change removed. The others
      // stay literal — they have not moved in months, and importing five
      // constants from four modules to state a merge policy reads worse than
      // the policy does.
      recent: mergeRing("recent", 20), seen: mergeRing("seen", SEEN_CAP),
...
export async function adoptAccountData(fromUserId, toUserId) {
  ...
  if (!p) {
    ...
    const run = withOrderedUserLocks([fromUserId, toUserId], async () => {
      ...
      // user_records (v52): the account's own record wins a collision, the
      // device's moves only into a gap — the same no-clobber rule as prefs.
      for (const key of [...mem.records.keys()]) {
        if (!key.startsWith(fromUserId + "|")) continue;
        const rest = key.slice(fromUserId.length + 1);
        const toKey = toUserId + "|" + rest;
        const row = mem.records.get(key);
        if (!mem.records.has(toKey)) mem.records.set(toKey, { ...row, userId: toUserId });
  ...
  try {
    ...
    const linkedCount = await client.query(
      `SELECT (
        ...
        (SELECT count(*) FROM user_records WHERE user_id=$1)
      )::int AS n`,
      [fromUserId]
    );
    ...
    // user_records (v52): no-clobber, then the device rows leave.
    await client.query(
      `INSERT INTO user_records (user_id, kind, record_id, payload, created_at, updated_at)
       SELECT $2, kind, record_id, payload, created_at, updated_at FROM user_records WHERE user_id=$1
       ON CONFLICT (user_id, kind, record_id) DO NOTHING`,
      [fromUserId, toUserId]);
    await client.query("DELETE FROM user_records WHERE user_id=$1", [fromUserId]);
