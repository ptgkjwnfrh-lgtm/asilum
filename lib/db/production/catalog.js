...
/**
 * The other photographs of one piece, isolated where a cutout exists.
 *
 * SAME RULE AS THE ITEM PAYLOAD (lib/products.js): `imageUrl` is what a surface
 * renders and prefers the matted version; `photograph` is what the seller
 * published and never disappears; `isolated` says which of the two this is, so
 * a gallery can give a cutout `contain` and no frame and give a photograph
 * `cover`. Selecting only `image_url` here would have left 7,878 stored cutouts
 * unreachable by the one function that reads this table.
 */
export async function getProductImages(productId) {
  const p = await getPool();
  if (!p) return [];
  const { rows } = await p.query(
    `SELECT image_url, alt_text, position, cutout_url
     FROM product_images WHERE product_id=$1 ORDER BY position`,
    [productId]
  );
  return rows.map((r) => ({
    imageUrl: r.cutout_url || r.image_url,
    photograph: r.image_url,
    isolated: !!r.cutout_url,
    altText: r.alt_text,
    position: r.position,
  }));
}
