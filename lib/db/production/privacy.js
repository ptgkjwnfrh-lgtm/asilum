...
import { listReflections } from "./stream.js";
...
export const EXPORT_MANIFEST = Object.freeze({
  ...
  // the system's account of why a send was answered so (schema v51) — the
  // person's own record, exported as rows and erased with them
  asterisk_reflections: { domain: "reflections", mode: "rows" },
...
export async function exportPersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  const [
    ...
    listReflections(userId, EXPORT_CAPS.reflections).catch(() => []),
  ]);
  ...
  const data = {
    ...
    reflections: bounded(reflections, EXPORT_CAPS.reflections),
    styleProfile,
...
export async function purgePersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  if (!p) {
    ...
    mem.reflections = mem.reflections.filter((row) => row.userId !== userId);
  ...
  try {
    ...
    // schema v51 may not be applied yet where this runs; erasure never
    // throws on an absent table, and never skips a present one
    const reflections = await client.query("SELECT to_regclass('public.asterisk_reflections') AS t");
    if (reflections.rows[0]?.t) await client.query("DELETE FROM asterisk_reflections WHERE user_id=$1", [userId]);
