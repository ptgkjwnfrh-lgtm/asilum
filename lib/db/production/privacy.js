...
import { listUserRecords } from "./records.js";
...
import { revokeMerchantConnectionFromProvider } from "./connections.js";
...
export const EXPORT_MANIFEST = Object.freeze({
  ...
  // v53: saves of people/places/events/articles, the base city and its
  // visibility, queued corrections and studio drafts — the person's own acts.
  user_records: { domain: "records", mode: "rows" },
  ...
  merchant_connections: { domain: "connections", mode: "rows" },
  items: { domain: "connectionCatalog", mode: "rows" },
  product_tags: { domain: "connectionCatalog", mode: "rows" },
  product_images: { domain: "connectionCatalog", mode: "rows" },
...
const EXPORT_CAPS = Object.freeze({
  records: 500,             // listUserRecords      boundedLimit(…, 500, 1000)
  ...
  dmMessages: 5000,
  dmBlocks: 500,
  connectionItems: 5000,
  connectionTags: 10000,
  connectionImages: 10000,
});
...
export async function exportPersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  const [
    ...
    listUserRecords(userId, { limit: EXPORT_CAPS.records }).catch(() => []),
  ]);
  ...
  const data = {
    ...
    records: bounded(records, EXPORT_CAPS.records),
    styleProfile,
    ...
    connections: [],
    connectionCatalog: {
      items: bounded([], EXPORT_CAPS.connectionItems),
      tags: bounded([], EXPORT_CAPS.connectionTags),
      images: bounded([], EXPORT_CAPS.connectionImages),
    },
  ...
  if (!p) {
    ...
    data.connections = accountId ? [...mem.merchantConnections.values()]
      .filter((row) => row.accountId === accountId)
      .map(({ tokenCiphertext, refreshTokenCiphertext, ...row }) => row) : [];
    if (accountId) {
      const ids = new Set(data.connections.map((row) => row.id));
      const items = [...coreMem.items.values()].filter((item) => ids.has(item.connection_id));
      const itemIds = new Set(items.map((item) => item.id));
      data.connectionCatalog = {
        items: bounded(items.slice(0, EXPORT_CAPS.connectionItems), EXPORT_CAPS.connectionItems),
        tags: bounded(mem.productTags.filter((tag) => itemIds.has(tag.productId)).slice(0, EXPORT_CAPS.connectionTags), EXPORT_CAPS.connectionTags),
        images: { ...bounded([], EXPORT_CAPS.connectionImages), store: "not mirrored in memory mode" },
      };
    }
    ...
    if (accountId) {
      const connections = await q(
        `SELECT id,provider,provider_account_id,canonical_domain,granted_scopes,
                publication_selection,status,imported_count,skipped_count,last_error,
                last_synced_at,created_at,updated_at,revoked_at
         FROM merchant_connections WHERE account_id=$1::uuid ORDER BY created_at DESC`,
        [accountId]);
      data.connections = connections.rows;
      const [catalogItems, catalogTags, catalogImages] = await Promise.all([
        q(`SELECT id,connection_id,source_name,source_product_id,title,brand,price,currency,
                  availability_status,resale_status,source_updated_at,availability_checked_at,expires_at
           FROM items WHERE connection_id IN (
             SELECT id::text FROM merchant_connections WHERE account_id=$1::uuid)
           ORDER BY id LIMIT $2`, [accountId, EXPORT_CAPS.connectionItems]),
        q(`SELECT t.product_id,t.tag,t.tag_type,t.confidence,t.source FROM product_tags t
           JOIN items i ON i.id=t.product_id JOIN merchant_connections c ON c.id::text=i.connection_id
           WHERE c.account_id=$1::uuid ORDER BY t.id LIMIT $2`, [accountId, EXPORT_CAPS.connectionTags]),
        q(`SELECT p.product_id,p.image_url,p.alt_text,p.position FROM product_images p
           JOIN items i ON i.id=p.product_id JOIN merchant_connections c ON c.id::text=i.connection_id
           WHERE c.account_id=$1::uuid ORDER BY p.id LIMIT $2`, [accountId, EXPORT_CAPS.connectionImages]),
      ]);
      data.connectionCatalog = {
        items: bounded(catalogItems.rows, EXPORT_CAPS.connectionItems),
        tags: bounded(catalogTags.rows, EXPORT_CAPS.connectionTags),
        images: bounded(catalogImages.rows, EXPORT_CAPS.connectionImages),
      };
    }
...
export async function purgePersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  if (!p) {
    ...
    for (const key of [...mem.records.keys()]) if (key.startsWith(userId + "|")) mem.records.delete(key);
    ...
    if (/^sb-[0-9a-f-]{36}$/i.test(userId)) {
      ...
      for (const [id, connection] of mem.merchantConnections) {
        if (connection.accountId !== accountId) continue;
        await revokeMerchantConnectionFromProvider(id);
  ...
  try {
    ...
    await client.query("DELETE FROM user_records WHERE user_id=$1", [userId]);
    ...
    if (/^sb-[0-9a-f-]{36}$/i.test(userId)) {
      const accountId = userId.slice(3);
      await client.query(
        `DELETE FROM product_tags WHERE product_id IN (
           SELECT i.id FROM items i JOIN merchant_connections c ON c.id::text=i.connection_id
           WHERE c.account_id=$1::uuid)`, [accountId]);
      await client.query(
        `DELETE FROM product_images WHERE product_id IN (
           SELECT i.id FROM items i JOIN merchant_connections c ON c.id::text=i.connection_id
           WHERE c.account_id=$1::uuid)`, [accountId]);
      await client.query(
        `DELETE FROM items WHERE connection_id IN (
           SELECT id::text FROM merchant_connections WHERE account_id=$1::uuid)`, [accountId]);
      await client.query(
        `UPDATE merchant_connections SET token_ciphertext=NULL,refresh_token_ciphertext=NULL,
         status='revoked',revoked_at=COALESCE(revoked_at,now()),updated_at=now()
         WHERE account_id=$1::uuid`, [accountId]);
      await client.query("DELETE FROM merchant_connections WHERE account_id=$1::uuid", [accountId]);
