...
export async function upsertItems(items = []) {
  ...
  const payload = rows.map((it) => ({
    ...
    listing_kind: it.listing_kind === "ghost" ? "ghost" : "live",
  }));
  await p.query(
    `INSERT INTO items
       (id,title,brand,price,currency,tags,img,alt,source,designers,category,era,size,url,
        ...
        material,fit,silhouette,condition,decade,color_evidence,is_available,availability_status,listing_kind,last_synced_at,updated_at)
     SELECT id,title,brand,price,currency,COALESCE(tags,'{}'::jsonb),img,alt,source,
            ...
            condition,decade,COALESCE(color_evidence,'{}'::jsonb),COALESCE(is_available,true),COALESCE(availability_status,'unknown'),COALESCE(listing_kind,'live'),now(),now()
     FROM jsonb_to_recordset($1::jsonb) AS x(
       ...
       is_available boolean,availability_status text,listing_kind text)
