...
export async function purgePersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  if (!p) {
    ...
    return { persistent: false, retained: ["purchase tickets", "deidentified raw event counters (eng/imp, no identity column)", "auth account"] };
  }
  ...
  try {
    ...
    return { persistent: true, retained: ["purchase tickets", "deidentified raw event counters (eng/imp, no identity column)", "auth account"] };
  } catch (error) {
...
const EXPORT_CAPS = Object.freeze({
  interactions: 5000, events: 5000, searches: 1000, wardrobe: 500,
  moodboard: 200, stylist: 200, corrections: 500, editorial: 200, aiEvents: 500,
});
...
export async function exportPersonalizationData(userId, { queryTarget = null } = {}) {
  ...
  const [
    ...
    listMoodBoardUploads(userId, EXPORT_CAPS.moodboard).catch(() => []),
    listMoodBoardAnalyses(userId, EXPORT_CAPS.moodboard).catch(() => []),
    listStylistOutfits(userId, EXPORT_CAPS.stylist).catch(() => []),
    listStylistFeedback(userId, EXPORT_CAPS.stylist).catch(() => []),
  ...
  const data = {
    ...
    moodboard: {
      uploads: bounded(uploads, EXPORT_CAPS.moodboard),
      analyses: bounded(analyses, EXPORT_CAPS.moodboard),
    },
    stylist: {
      outfits: bounded(outfits, EXPORT_CAPS.stylist),
      feedback: bounded(stylistFb, EXPORT_CAPS.stylist),
      requests: bounded([], EXPORT_CAPS.stylist),
    },
  ...
  if (!p) {
    ...
    data.stylist.requests = bounded(
      mem.stylistRequests.filter((r) => r.userId === userId).slice(-EXPORT_CAPS.stylist).reverse(),
      EXPORT_CAPS.stylist);
    ...
    const [searches, requests, aiEvents, editorial, interp, room, corro, internal] = await Promise.all([
      ...
      q(`SELECT id, created_at FROM stylist_requests WHERE user_id=$1
         ORDER BY created_at DESC LIMIT $2`, [userId, EXPORT_CAPS.stylist]),
    ...
    data.stylist.requests = bounded(requests.rows, EXPORT_CAPS.stylist);
  ...
  return {
    ...
    retained: ["purchase tickets", "deidentified raw event counters (eng/imp, no identity column)", "auth account"],
    countedNotDetailed: {
...
export async function listEditorialPosts({ kind = null, limit = 60 } = {}) {
  ...
  if (!p) {
    return mem.posts
      .filter((x) => (x.moderationStatus ?? "visible") === "visible" && (!kind || x.kind === kind))
      .slice(-limit).reverse();
  }
  const { rows } = await p.query(
