...
export async function createMoodBoardUpload(u) {
  ...
  const row = {
    ...
    analyzedBy: ["none", "filename", "manual", "vision"].includes(u.analyzedBy) ? u.analyzedBy : "none",
  };
  const p = await getPool();
  if (!p) return push(mem.uploads, { id: mem.seq++, ...row, createdAt: Date.now(), persistent: false });
  const { rows } = await p.query(
    `INSERT INTO mood_board_uploads
       (user_id, board_id, image_url, caption, source, colors, tags, style_notes, analyzed_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, created_at`,
    [row.userId, row.boardId, row.imageUrl, row.caption, row.source,
     JSON.stringify(row.colors), JSON.stringify(row.tags), row.styleNotes, row.analyzedBy]
  );
  return { id: rows[0].id, ...row, createdAt: new Date(rows[0].created_at).getTime(), persistent: true };
}
...
export async function saveStylistOutfit(o) {
  ...
  const { rows } = await p.query(
    ...
    [row.userId, row.signature, row.genre, JSON.stringify(row.items), row.matchScore, JSON.stringify(row.reasons)]
  );
  return { id: rows[0].id, ...row, createdAt: new Date(rows[0].created_at).getTime(), persistent: true };
}
