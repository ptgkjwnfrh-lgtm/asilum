...
export async function setFollow(userId, kind, target, on = true) {
  ...
  if (typeof target !== "string" || !target.length || target.length > 120) {
    throw new TypeError("target must be a 1-120 char string");
  }
  const p = await getPool();
  if (!p) {
    mem.follows = mem.follows.filter((f) => !(f.userId === userId && f.kind === kind && f.target === target));
    if (on) {
      const existing = mem.follows.filter((f) => f.userId === userId);
      if (existing.length >= FOLLOWS_CAP) return { ok: false, error: "follow cap reached" };
      push(mem.follows, { userId, kind, target, createdAt: Date.now() });
    }
    return { ok: true };
  }
  if (!on) {
    await p.query("DELETE FROM user_follows WHERE user_id=$1 AND kind=$2 AND target=$3", [userId, kind, target]);
    return { ok: true };
  }
  const { rows } = await p.query("SELECT count(*)::int AS n FROM user_follows WHERE user_id=$1", [userId]);
  if ((rows[0]?.n || 0) >= FOLLOWS_CAP) return { ok: false, error: "follow cap reached" };
  await p.query(
