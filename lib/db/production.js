...
// Uniqueness is scoped per user (Codex #13 follow-up): the old global index
// let one user's key collide with another's, so it is replaced then dropped.
...
function ensureIdemColumn(p) {
  if (!idemColumnReady) {
    idemColumnReady = p
      ...
      .then(() => p.query("DROP INDEX IF EXISTS mood_board_uploads_idem"))
...
export async function adoptAccountData(fromUserId, toUserId) {
  if (!fromUserId || !toUserId || fromUserId === toUserId) {
    return { movedProfile: false, movedBoards: 0, movedCorrections: 0, duplicate: true };
  }
  const p = await getPool();
  ...
  if (!p) {
    ...
    const run = withUserLock(toUserId, async () => {
      ...
      return { movedProfile, movedBoards, movedCorrections, duplicate: false };
    });
