...
export async function productsByTagsTyped(tags = [], { limit = 500 } = {}) {
  ...
  const { rows } = await p.query(
    `SELECT product_id, tag_type, sum(confidence) AS score FROM product_tags
     WHERE tag = ANY($1) GROUP BY product_id, tag_type
...
export async function listProfileThemes() {
  const p = await getPool();
...
export async function getProfileRoom(accountId) {
...
export async function upsertProfileRoom(accountId, { handle, themeId, published } = {}) {
  const id = requireAccountId(accountId);
  const patch = {};
...
export async function setProfileRoomModeration(accountId, status) {
  const id = requireAccountId(accountId);
...
export async function setProfileModule(accountId, module, { position, visible, content } = {}) {
  const id = requireAccountId(accountId);
  ...
  if (!p) {
    ...
    const current = mem.profileModules.get(key) || {
      accountId: id, module, position: 100, visible: true, content: {}, updatedAt: Date.now(),
    };
    const next = { ...current, ...patch, updatedAt: Date.now() };
  ...
  try {
    const { rows } = await p.query(
      `INSERT INTO profile_modules (account_id, module, position, visible, content, updated_at)
       VALUES ($1, $2, COALESCE($3,100), COALESCE($4,true), COALESCE($5,'{}'::jsonb), now())
       ON CONFLICT (account_id, module) DO UPDATE SET
         position   = COALESCE($3, profile_modules.position),
         visible    = COALESCE($4, profile_modules.visible),
...
export async function applyBrandCaseTransition(id, expectedStatus, t) {
  ...
  try {
    ...
    if (!rows[0]) {
      await client.query("ROLLBACK");
      const exists = await getBrandCase(id);
...
export async function listBrandCaseEvents(caseId, limit = 100) {
  ...
  const { rows } = await p.query(
    "SELECT case_id, from_status, to_status, actor, note, at FROM brand_case_events WHERE case_id=$1 ORDER BY at, id LIMIT $2",
    [caseId, cap]);
  return rows.map((r) => ({
