...
export async function updateEditorialPost({ id, authorId, title = null, body = null, excerpt = null, moderationStatus = "visible" }) {
  ...
  if (!p) {
    ...
    if (!row) return null;
    Object.assign(row, next, { moderationStatus: status, editedAt: Date.now() });
    return { ...row };
  }
  const { rows } = await p.query(
