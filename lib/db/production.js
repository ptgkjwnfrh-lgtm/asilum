...
export async function setWardrobePhoto(userId, id, photoPath, consentVersion) {
  const p = await getPool();
  if (!p) {
    const r = mem.wardrobe.find((w) => w.user_id === userId && String(w.id) === String(id));
    if (!r) return null;
  ...
  if (!/^\d{1,18}$/.test(String(id))) return null;
  const { rows } = await p.query(
    `UPDATE wardrobe_items SET photo_path=$3, photo_consent=$4, updated_at=now()
     WHERE user_id=$1 AND id=$2 RETURNING *`,
    [userId, id, photoPath, photoPath ? consentVersion : null]);
  return rows[0] ? wardrobeRow(rows[0]) : null;
}

// Palette colors land on the row only when the user hasn't set any — the
// photo never overrides explicit data.
export async function setWardrobeColorsIfEmpty(userId, id, colors) {
  if (!Array.isArray(colors) || !colors.length) return null;
  const p = await getPool();
  if (!p) {
    const r = mem.wardrobe.find((w) => w.user_id === userId && String(w.id) === String(id));
    if (!r || (Array.isArray(r.colors) && r.colors.length)) return null;
    r.colors = colors.slice(0, 8);
    return wardrobeRow(r);
  }
  if (!/^\d{1,18}$/.test(String(id))) return null;
  const { rows } = await p.query(
    `UPDATE wardrobe_items SET colors=$3::jsonb, updated_at=now()
     WHERE user_id=$1 AND id=$2 AND jsonb_array_length(colors)=0 RETURNING *`,
    [userId, id, JSON.stringify(colors.slice(0, 8))]);
  return rows[0] ? wardrobeRow(rows[0]) : null;
}
