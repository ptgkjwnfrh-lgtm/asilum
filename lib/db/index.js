...
const REQUIRED_TABLES = [
  ...
  "user_measurements", "asterisk_memory_preferences", "user_follows",
];
  ...
  const { rows } = await p.query(
    `WITH query AS (SELECT websearch_to_tsquery('simple', $1) AS value)
     ...
     WHERE i.moderation_status='visible'
       ...
       AND (
         i.search_document @@ query.value
         OR lower(i.brand)=lower($1)
         OR EXISTS (
           SELECT 1 FROM product_tags AS pt
           WHERE pt.product_id=i.id AND pt.tag=ANY($2::text[])
         )
       )
