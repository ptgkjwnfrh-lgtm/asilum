...
export async function commitInteractionBatch({ userId, operationId, events, reduce }) {
  ...
  if (!p) {
    return withUserLock(userId, async () => {
      ...
      await Promise.all([bumpEdges(derived.edgePairs, userId), bumpPopularity(derived.popularity)]);
...
async function writeEdges(target, pairs = [], userId = null) {
  ...
  const hash = identityHash(userId);
  // `edges` becomes a MATERIALIZED VIEW of the ledger: w is the sum of each
  // contributor's best weight for the pair and contributors is the distinct
  // count, both recomputed from edge_contributors for exactly the pairs this
  // request touched. Recompute rather than delta-arithmetic because a repeat
  // contributor RAISING their best (favorite→bag) must move w by the
  // difference, and ON CONFLICT cannot see the pre-update value in RETURNING.
  // It is idempotent by construction, and it heals the historical rows: a
  // poisoned w is REPLACED by the honest ledger sum the first time the pair
  // is touched again, instead of being inherited forever.
  //
  ...
  // may always update their own row.
  await target.query(
    `WITH input AS (
       ...
       SELECT DISTINCT a,b FROM ins
     ), agg AS (
       SELECT c.a, c.b, sum(c.w)::real AS w, count(*)::int AS contributors
       FROM edge_contributors c JOIN touched t ON t.a = c.a AND t.b = c.b
       GROUP BY c.a, c.b
     )
     INSERT INTO edges (a,b,w,contributors)
     SELECT a, b, w, LEAST($3::int, contributors) FROM agg
...
export async function getPopularity(limit = 5000) {
  limit = Math.max(1, Math.min(10_000, Math.trunc(Number(limit)) || 5000));
  const p = await getPool();
  const out = {};
  if (!p) {
