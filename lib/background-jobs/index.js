...
import { runCatalogWorker } from "../connections/shopifyCatalog.js";

const job = (id, description, needs) => ({
  id, description, needs, runnable: false,
  run: async () => notImplemented("job:" + id, "no job runner in the stack; " + needs),
});

const runnableJob = (id, description, needs, run) => ({ id, description, needs, runnable: true, run });
...
export const JOBS = [
  ...
  runnableJob("sync-external-accounts", "refresh opt-in merchant catalog connections", "schema v51 + configured merchant connectors", runCatalogWorker),
...
/** What the scheduled jobs are and whether each is actually armed. Reports
 *  only what it can verify — a job with no schedule reads as off, not pending. */
export function jobsStatus() {
  return JOBS.map(({ id, description, needs, runnable }) => ({ id, description, needs, runnable }));
}
