...
// Taste ORDERS screen/soundtrack entities (never filters — same doctrine as
// the query router); rotation is deterministic by UTC day, not engagement
// bait. Per-user prefs (collapse/hide) are the only writes.
...
import { getDiscoverablePool, publicProduct } from "../products.js";
import { listDiscoverRails, getRailPrefs } from "../db/production.js";
...
export async function discoverRails(userId = null, now = new Date()) {
  const [registry, prefs, styleProfile] = await Promise.all([
    listDiscoverRails(),
    userId ? getRailPrefs(userId) : Promise.resolve({}),
    userId ? getUserStyleProfile(userId).catch(() => null) : Promise.resolve(null),
  ]);
  ...
  for (const rail of registry) {
    ...
    else if (rail.kind === "exploration") rails.push({ ...base, ...(await explorationRail(userId, day)) });
  }
  return {
    rails,
    rotatesDaily: true,
