...
/** Feature flag: is the brand verification case system open? OFF unless
 *  `BRAND_VERIFICATION_ENABLED=1`. */
...
/**
 * Validate a new brand case before it may be opened. THROWS on anything
 * invalid.
 *
 * Throwing rather than returning an error is right here: brand_cases is an
 * APPEND-ONLY enforcement ledger, so a malformed row cannot be deleted once
 * written. `openedBy` is required because every case names the person who
 * opened it.
 */
