...
export const GROUPS = [
  ...
  },
  {
    ...
    controls: [
      ...
      { key: "--ed-fs-tbtn", label: "top bar buttons", unit: "px", min: 8, max: 16, step: 0.5, fallback: 10.5, selectors: [".tbtn", ".topright"] },
  ...
  },
  {
    ...
    title: "LAYOUT & SHAPE",
    controls: [
      { key: "--ed-side-w", label: "sidebar width", unit: "px", min: 170, max: 340, step: 2, fallback: 232, selectors: [".side"] },
      { key: "--ed-mq-h", label: "ticker height", unit: "px", min: 30, max: 70, step: 1, fallback: 44, selectors: [".marquee"] },
      ...
      { key: "--ed-card-gap", label: "card stack gap", unit: "px", min: 8, max: 60, step: 2, fallback: 26, selectors: [".card", ".post"] },
      { key: "--ed-card-rad", label: "card corner", unit: "px", min: 0, max: 28, step: 1, fallback: 12, selectors: [".card"] },
      { key: "--ed-img-rad", label: "image corner", unit: "px", min: 0, max: 24, step: 1, fallback: 10, selectors: [".imgwrap"] },
      { key: "--ed-panel-rad", label: "panel corner", unit: "px", min: 0, max: 28, step: 1, fallback: 14, selectors: [".panel", ".searchpanel", ".bagpanel"] },
      { key: "--ed-snav-pad-y", label: "nav item pad ↕", unit: "px", min: 4, max: 20, step: 1, fallback: 9, selectors: [".snav"] },
  ...
  },
  {
    ...
    title: "ATMOSPHERE",
    controls: [
      { key: "--ed-grid-cell", label: "backdrop grid cell", unit: "px", min: 20, max: 120, step: 2, fallback: 44, selectors: ["body"] },
  ...
  },
  {
    ...
    controls: [
      ...
      { key: "boot", label: "boot sweep", kind: "boot", note: "the X/Y sweep on every load", selectors: [".os-boot"] },
    ],
  },
];
...
const ALLOWED_KEYS = new Set(
  ALL_CONTROLS.filter((c) => c.kind !== "boot").map((c) => c.key)
);
...
export function bootOn() {
  try { return window.localStorage.getItem("asilum-boot") !== "off"; } catch { return true; }
}

export function setBoot(on) {
  try {
    if (on) window.localStorage.removeItem("asilum-boot");
    else window.localStorage.setItem("asilum-boot", "off");
  } catch {}
}
