...
//
// What moved where (V.2): CATALOG became the PIECES tab of DISCOVER (the
// ranked grid at "/" IS discovery); FULL READ (/stats) and UPLOAD ride inside
// PASSPORT as tabs; PROFILE, ORDERS and SETTINGS live under the account
// circle at the top right (ACCOUNT_MENU below) — one identity, one door.
...
const NAV = [
  ...
  { href: "/", icon: "◎", label: "DISCOVER", meta: "PIECES · SEARCH · PLACES · PEOPLE",
    match: (p) => p === "/" || p.startsWith("/discover") || p.startsWith("/stylist") || p.startsWith("/piece") },
  { href: "/hotlist", icon: "✎", label: "THE WIRE", meta: "PURCHASABLE CULTURE",
    match: (p) => p.startsWith("/hotlist") || p.startsWith("/u/") },
