...
/** Edit distance between two strings. Used for fuzzy suggestion matching only
 *  — never for scoring results, where a near-miss must be an explicit typo
 *  correction rather than a quietly weakened match. */
...
/**
 * Autocomplete for a partial query: corrected spellings, related terms,
 * designers, cultural entities.
 *
 * A SUGGESTION IS A PROMISE THAT SEARCHING IT WILL LAND SOMEWHERE — the
 * vocabulary is built from what the engine can actually resolve, so nothing
 * offered here dead-ends. Typing is not a committed search and is never
 * logged.
 */
