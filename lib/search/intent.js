...
export function getSearchIntent(query, brands = [], { brandSpelling = true } = {}) {
  ...
  // MEASURED on the live catalog: a listing branded "Black" made bare "black"
  // a brand query, and "leather" resolved to "casablanca leather" through the
  // partial rule below. A bare colour / material / gender / garment word is a
  // description, whatever a marketplace brand field says.
  if (isOrdinaryWord(q)) return { intent: "text" };
