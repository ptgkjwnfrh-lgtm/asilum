...
export async function searchProducts(query, { userId = null, brain = false, limit = 48, offset = 0 } = {}) {
  ...
  let ranked = rankSearchResults(pool, rankedInterpreted, { tagLayerScores, personal });
  ...
  // Dense recall (embeddings v1, asterisk-boost r4): when a provider is
  // keyed AND the catalog has been backfilled (scripts/embed-catalog.mjs),
  // semantic neighbors of the query join the results the literal engine
  // missed. Additive and bounded: existing results keep their order; vector
  // hits append below with matchReason "semantic match" and capped
  // confidence. Zero effect while unkeyed — the paths above are untouched.
    ...
    try {
      ...
      if (qe.ok && stored.length) {
        ...
        for (const row of stored) {
          if (have.has(row.owner_id)) continue;
          const it = byId.get(row.owner_id);
          if (!it) continue;
          const sim = cosine(qe.data.vector, row.vector);
          if (sim > 0.45) hits.push({ it, sim });
        }
