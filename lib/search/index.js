...
const GENERIC_GARMENT_NOUNS = {
  jacket: "outerwear", coat: "outerwear",
  shoes: "footwear",
  dress: "dresses",
  // (r8) every knitwear item IS a knit — turtlenecks, cardigans, crewnecks,
  // mohair sweaters, even balaclava beanies. "sweater" was measured and
  // REJECTED: the declared beanie guard fired (balaclava beanies topped
  // "sweater for a freezing mountain hut" — semantically great winter gear,
  // not sweaters). Re-adding it is blocked on the catalog fix that moves
  // balaclava beanies out of knitwear. "boots" stays out (subtype of
  // footwear); "top" stays out (the tokenizer splits "high-top" into
  // [high, top] and would penalize actual high-tops on their own query).
  knit: "knitwear",
};
...
export function rankSearchResults(products, interpreted, { tagLayerScores = {}, personal = null, garmentTitleEquiv = true } = {}) {
  ...
  for (const it of products) {
    ...
    const equivHit = (t) => {
      ...
      const c = GENERIC_GARMENT_NOUNS[t] || GENERIC_GARMENT_NOUNS[t.replace(/s$/, "")];
...
export async function searchProducts(query, {
  ...
  const tagQueryTokens = garmentTitleEquiv
    ? dense.tokens.filter((t) => !(GENERIC_GARMENT_NOUNS[t] || GENERIC_GARMENT_NOUNS[t.replace(/s$/, "")]))
    : dense.tokens;
  let tagLayerScores = {};
  try {
  ...
  if ((!ranked.length || literalWeak) && dense.tokens.length && pool.length) {
    ...
    if (readTags.length) {
      const wantCats = new Set(
        dense.tokens
          .map((t) => GARMENT_CATEGORY[t] || GARMENT_CATEGORY[t.replace(/s$/, "")])
          .filter(Boolean)
      );
      const composed = [];
      for (const it of pool) {
  ...
  if (embeddingsConfigured()) {
    try {
      const isGarment = (t) => !!(GARMENT_CATEGORY[t] || GARMENT_CATEGORY[t.replace(/s$/, "")]);
