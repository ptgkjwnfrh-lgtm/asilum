...
import { listSearchMappings, productsByTags, logSearch as dbLogSearch } from "../db/production.js";
...
export async function getProductPool(limit = 1000) {
  let pool = [];
  try { pool = await listItems(limit); } catch { pool = []; }
  const ids = new Set(pool.map((it) => it.id));
  // Seed catalog is the emergency development fallback only — merged just for
  // ids the database doesn't have (empty DB = keyless dev run).
  return [...pool, ...CATALOG.filter((it) => !ids.has(it.id))];
}
...
export async function interpretSearchQuery(query, context = {}) {
  ...
  const brands = [...new Set(pool.map((it) => it.brand).filter(Boolean))];
...
export async function searchProducts(query, { userId = null, brain = false, limit = 48, offset = 0 } = {}) {
  ...
  const interpreted = await interpretSearchQuery(q, { pool, mappings });
  let tagLayerScores = {};
  try {
    tagLayerScores = await productsByTags([...interpreted.mappedTags, ...interpreted.tokens, q]);
  } catch { tagLayerScores = {}; }
  const personal = brain ? await getPersonalizedSearchContext(userId) : null;
  const ranked = rankSearchResults(pool, interpreted, { tagLayerScores, personal });
  ...
  return {
    ...
    total: ranked.length,
    results: ranked.slice(offset, offset + limit).map(({ _score, ...it }) => it),
  };
}
