...
export async function searchProducts(query, {
  ...
  // they were read as.
  if (!note && eraConstraint) {
    const read = `reading "${eraConstraint.words.join(" ")}" as ${eraConstraint.label}`;
    // An era that WAS served but left an empty rack means the rest of the
    // query found nothing inside it — say which half worked.
  ...
  if (unmatched.length && ranked.length && !cultural.engaged) {
    ...
    const shownCats = [...new Set(matchedWords
      ...
      .filter(Boolean))];
    const said = `no piece here matches "${unmatched.join('", "')}"`;
    const scoped = shownCats.length
      ? `${said} — showing ${shownCats.join(" / ")} instead`
      : matchedWords.length ? `${said} — showing ${matchedWords.join(" ")} instead` : said;
    note = note ? `${note}; ${scoped}` : scoped;
  }
