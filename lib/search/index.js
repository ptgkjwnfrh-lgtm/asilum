...
import { parseEraConstraint, applyEraConstraint, eraMissNote } from "./era.js";
import { parseOriginConstraint, applyOriginConstraint, originMissNote } from "./origin.js";
...
export function rankSearchResults(products, interpreted, { tagLayerScores = {}, personal = null, garmentTitleEquiv = true } = {}) {
  ...
  for (const it of products) {
    ...
    // Token title matching is WORD-BOUNDARY, singular-or-plural. Bare
    // substring let "ross" claim a title match inside "crossbody", "tens"→
    // "ten" inside "Noten", "nas"→"na" inside "anorak" (Aug 5 vibe sweep) —
    // junk evidence that also blocked the cultural tier by making a rack
    // look literally grounded. The full-phrase product-name match above is
    // unchanged.
    const esc = (x) => x.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const wordHit = (x) => new RegExp("\\b" + esc(x) + "\\b").test(title);
    const stripHit = (t) => t.endsWith("s") && t.length >= 4 && wordHit(t.slice(0, -1));
    const hitsTitle = (t) => wordHit(t) || stripHit(t) || equivHit(t);
    ...
    if (q && new RegExp("\\b" + esc(q) + "\\b").test(title)) { score += 6; reasons.push(["product name match", 6]); }
    ...
    for (const t of qTokens) {
      if (wordHit(t) || stripHit(t)) tokenEvidence.add(t);
      else if (equivHit(t) || garmentCategoryOf(t)) tokenEvidence.add(t);
      else if (brand.includes(t) || mappedUpper.includes(t.toUpperCase())) tokenEvidence.add(t);
    ...
    scored.push({
      ...
      matchReason: categoryOnly ? "category browse" : (top ? top[0] : "match"),
...
export async function searchProducts(query, {
  ...
  if ((!ranked.length || literalWeak) && !cultureHit) composeRead();
  ...
  if ((!ranked.length || !queryGrounded || (cultureHit && literalWeak)) && pool.length && process.env.SEARCH_CULTURE_FALLBACK !== "0") {
    try {
  ...
  if (cultureHit && !cultural.engaged && (!ranked.length || literalWeak)) composeRead();
  ...
  const anyConstraint = !!(
    eraConstraint || originConstraint || dense.constraints.gender ||
    dense.constraints.climate ||
  ...
  if (unmatched.length && ranked.length && !cultural.engaged) {
    ...
    const wordScope = buildTitleWordScope(await getProductPool());
    const elsewhere = unmatched.filter((t) => categoriesCarrying(t, wordScope, shownCats).length);
    ...
    if (elsewhere.length) {
      ...
      clauses.push(`no "${elsewhere.join('", "')}"${where} — ` +
        elsewhereClause(elsewhere, wordScope, shownCats));
    }
  ...
  // every rate by the size of the tail.
