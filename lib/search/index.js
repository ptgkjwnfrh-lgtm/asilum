...
export async function searchProducts(query, {
  ...
  if (interleaved) {
    const said = `showing ${wantCats.join(" and ")} side by side — asterisk holds no data on what goes with what`;
    note = note ? `${said}; ${note}` : said;
  }

  // A sort is named, so the first row is not mistaken for the most relevant.
  if (priceSortSpec && ranked.length) {
    const said = `sorted by ${priceSortSpec.label}`;
    note = note ? `${said}; ${note}` : said;
  }
