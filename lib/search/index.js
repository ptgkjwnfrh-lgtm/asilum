...
import { houseForShortForm, houseForKbKey } from "../asterisk/houses.js";
...
export async function searchProducts(query, {
  ...
  const groundedFully = () => {
    ...
    if (designerCreditName) return true;
  ...
  const namesOnly = !!houseSet && !dense.tokens.length;
  ...
  const composeRead = () => {
    ...
    // A HOUSE NAME IS NOT A VIBE (11 Sep 2026). The origin register grew from
    // 63 houses to the archive canon, which put houses in the brain's key
    // space that this catalog does not stock — and the compositional tier
    // then read "telfar" as its AESTHETICS and served 24 lookalikes, where
    // the engine used to serve nothing. That is the invention this layer is
    // forbidden to make: the reader asked for a house, and a rack of pieces
    // that merely resemble it is an answer to a question nobody asked. Same
    // rule the short-form table already states — "a form for a house nobody
    // carries resolves to nothing" (lib/asterisk/houses.js). The house stays
    // in the brain for TASTE, where naming Telfar on /upload should still
    // teach the profile; it is only barred from standing in for stock.
    const asHouse = houseForKbKey(dense.tokens.join(" "));
    if (asHouse && !brands.some((b) => norm(b) === norm(asHouse))) return;
  ...
  const anyConstraint = !!(
    eraConstraint || originConstraint || designerCreditName || sizeConstraint || houseSet ||
    dense.constraints.gender || dense.constraints.climate ||
    dense.constraints.minPrice != null || dense.constraints.maxPrice != null ||
