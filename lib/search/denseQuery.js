...
import { itemMatchesGender } from "./gender.js";
import { FACET_WEIGHTS } from "../tagging/vocabulary.js";

// DERIVED, NOT MAINTAINED BESIDE THE WRITERS. This table used to be a second
// list of facet names kept by hand, and it had drifted: it knew the eighteen
// facets one writer emitted and none of the five the other did — color,
// condition, fabric, silhouette, subcategory all fell to the unknown floor
// below. A wool trouser scored 1.35 or 0.18 depending on which path tagged it.
// One register now, in lib/tagging/vocabulary.js, with the aliases resolved at
// write time so this side never sees two names for one axis.
export const TYPE_WEIGHTS = FACET_WEIGHTS;
...
export const GENDER_WORDS = Object.assign(Object.create(null), {
...
export function applyDenseConstraints(items = [], constraints = {}) {
  ...
  if (constraints.gender) {
    // Read through lib/search/gender.js (20 Sep 2026): a seller's "women's"
    // in the title outranks the size record's default "unisex".
    out = out.filter((it) => itemMatchesGender(it, constraints.gender));
  }
