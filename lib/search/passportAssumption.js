...
import { garmentCategoryOf } from "./index.js";
import { rankByInterpretationTags } from "../discover/tagRank.js";

const MIN_KEEP = 6;

/** Feature flag: may search assume the reader's passport context (their size,
 *  their measurements) when the query does not say? */
...
const hasGarmentNoun = (q) => String(q).toLowerCase().split(/[^a-z]+/).some(
  (t) => t.length > 1 && garmentCategoryOf(t)
);
 ...
 */
export async function resolveSearchAssumption(query, userId, feedback = []) {
  const out = {
  ...
  let interp = null;
  // feedback carries this user's prior verdicts so a reading they rejected in
  // the discover pills is not silently re-applied to their search (audit #6:
  // search never received recorded rejections). Empty array = no suppression.
  try { interp = await orchestrateInterpretation(query, userId, feedback); } catch { return { ...out, reason: "interpretation-error" }; }
