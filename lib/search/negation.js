...
//
// A NEGATION REACHES EVERY READER (Aug 22). Until now an exclusion could only
// ever be a NAME, so every other thing the engine can read was invisible to
// it and the result was the opposite of the request:
//
...
//
// A marker is followed by a CONSTRAINT PHRASE at least as often as by a name,
// so the parsers the engine already owns get first refusal on what follows a
// marker; only what none of them recognises falls through to a name.
//
 ...
 */
export function parseNegations(tokens = [], { phrases = [], readers = [] } = {}) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // THE READERS GET FIRST REFUSAL. Whatever follows the marker is offered
    // to the engine's own parsers before it is treated as a bare name.
    let handled = null;
    for (const reader of readers) {
      const hit = reader.parse(tokens.slice(j));
      if (hit && hit.consumed > 0) { handled = { reader, hit }; break; }
    }
    if (handled) {
      const { reader, hit } = handled;
      exclusions.push({
        kind: reader.kind, marker,
        word: tokens.slice(j, j + hit.consumed).map((t) => foldNorm(t)).join(" "),
    ...
    exclusions.push({ kind: "name", word: span.join(" "), marker });
    i = j + span.length - 1;
  }
...
/**
 * A constraint exclusion carries its own predicate (built by the caller from
 * the same reader that parsed it); a name exclusion uses the name matcher.
 */
export function exclusionMatches(item, x, opts = {}) {
  if (x && x.kind && x.kind !== "name") {
    return typeof x.test === "function" ? !!x.test(item) : false;
  }
  return itemMatchesExclusion(item, x.word, opts);
}

/**
 * Remove everything the reader excluded — "no leather", "not Balenciaga".
 *
 * A NEGATION IS ABSOLUTE. It filters rather than down-weighting, because a
 * reader who said "no leather" has stated a requirement, and a merely lowered
 * score still puts leather on the shelf.
 *
 * Pair with exclusionNote, which says what was removed AND on what basis — the
 * catalog can check a name but not a fabric, and the sentence admits which.
 */
export function applyExclusions(items = [], exclusions = [], opts = {}) {
  if (!exclusions.length) return items;
  return items.filter((it) => !exclusions.some((x) => exclusionMatches(it, x, opts)));
}
 ...
 */
export function exclusionNote(exclusions = [], removed = 0, remaining = 0, { scopeLabel = null } = {}) {
  if (!exclusions.length) return null;
  const named = exclusions.every((x) => !x.kind || x.kind === "name");
  const what = exclusions
    .map((x) => (x.label ? x.label : `"${x.word}"`))
    .join(" and ");
  const where = scopeLabel ? ` in ${scopeLabel}` : " here";
  if (!removed) {
    return named
      ? `nothing${where} is named ${what} — nothing to exclude`
      : `nothing${where} is ${what} — nothing to exclude`;
  }
  if (!remaining) return `every piece here is ${what} — nothing left after excluding it`;
  const basis = named ? " by name" : "";
  return `excluding ${what}${basis} — ${removed} piece${removed === 1 ? "" : "s"} removed`;
}
