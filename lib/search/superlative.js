...
export function parsePriceSuperlative(query, tokens = []) {
  ...
  for (const [phrase, direction] of all) {
    ...
    // "not cheapest jacket" is not a request to sort cheapest-first. The
    // caller strips negated spans before calling, and this is the belt to
    // that pair of braces.
    if (new RegExp(`\\b(?:no|not|without|except|excluding|minus|sans)\\s+(?:the\\s+)?${esc(phrase)}\\b`).test(q)) continue;
    return {
