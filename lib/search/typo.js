...
function catalogWords() {
  ...
  for (const it of CATALOG) {
    for (const src of [it.title, it.brand]) {
      for (const t of foldNorm(src).split(/[^a-z0-9]+/)) if (t.length > 2) w.add(t);
    }
  }
...
function cultureWords() {
  ...
  for (const e of CULTURE) {
    for (const key of [e.name, ...(e.aliases || [])]) {
      for (const t of foldNorm(key).split(/[^a-z0-9]+/)) if (t.length > 2) w.add(t);
    }
  }
...
const NEVER_CORRECT = new Set([
  ...
  // Ordinary English a shopper uses AROUND the clothes. MEASURED misfires,
  // each one a real query turned into a claim about a material the person
  // never mentioned: weather→leather ("hot weather trousers", "cold weather
  // boots"), print→point ("shirt minus the print"), stock→sock ("in stock
  // jacket"). The words nearest them in the same trap are listed with them.
  "weather", "print", "stock", "season", "sample", "thread", "quality",
  "price", "brand", "label", "model", "style", "piece", "store", "order",
  "small", "large", "short", "light", "heavy", "plain", "smart", "sharp",
  "loose", "tight", "clean", "worn", "warm", "cool", "thick", "thin",
]);

// A typo does not usually start at the first character, and a bridge that
// rewrites the opening letter is not correcting a slip — it is proposing a
// different word. MEASURED: every one of the corruptions above that survived
// the stoplist for months was a first-letter substitution (weather→leather).
// Requiring the initial to hold costs the bridge nothing it should have been
// doing: sweter→sweater, trousrs→trousers, jaket→jacket all keep their first
// letter, and so does every correction the suite pins.
const sameInitial = (a, b) => a.charCodeAt(0) === b.charCodeAt(0);

/** Feature flag: may unknown query tokens be corrected before lookup?
 *  ON unless SEARCH_TYPO_BRIDGE=0. */
...
export function isProtectedWord(token) {
  ...
  return NEVER_CORRECT.has(t) || ERA_WORDS.has(t) || ORIGIN_WORDS.has(t) ||
    cultureWords().has(t) || catalogWords().has(t);
}

/**
 * Correct unknown tokens against the known vocabulary, leaving known ones
 * untouched.
 *
 * Returns `{ tokens, corrections }` — the corrections travel WITH the result
 * so the reader can be told "showing results for…" rather than silently given
 * answers to a question they did not ask. A word the engine already knows is
 * never "corrected", which is what stops a real brand name being rewritten
 * into a more common one.
 */
