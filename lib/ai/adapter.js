...
//
// EXPORTED SO THE SUCCESS PATH IS TESTABLE. tests/ai-adapter.test.js recorded
// the gap in its own header: "PROVIDERS is a module-private const with no
// injection seam… the parse/validate/log branches of the most important
// function in lib/ai remain unverified". Three of four providers throw before
// any socket opens, so every REFUSAL was covered and nothing else was. A test
// swaps an entry, exercises parse/validate/log, and restores it. Nothing in
// the app writes to this object; a provider still only runs when the master
// switch, the per-feature switch, a provider name and a key are all present.
...
async function callAnthropic({ prompt, modelName, apiKey }) {
  ...
  try {
    ...
    if (!res.ok) {
      const body = await readResponseSnippet(res);
      throw new Error(`anthropic: HTTP ${res.status} ${body.slice(0, 200)}`);
    }
    const json = await readJsonResponse(res, { maxBytes: 2 * 1024 * 1024 });
...
export async function runModel(req) {
  ...
  try {
    // THE PROVIDER IS CHECKED BEFORE THE BUDGET IS SPENT. It used to be
    // checked after both quota calls, so a misconfigured AI_PROVIDER burned
    // the hourly allowance of every user and of the whole install before
    // failing on a name that was never going to work.
    ...
    if (!userQuota.allowed) {
      // A refusal is an event. Without this row the ledger showed a silent
      // gap where a real request had been made and declined, and nobody
      // reading ai_model_events could tell a quiet hour from a throttled one.
      await logAiModelEvent({ ...base, status: "rate-limited",
        inputSummary: summarize(context), errorMessage: "per-user hourly quota" }).catch(() => {});
    ...
    if (!globalQuota.allowed) {
      await logAiModelEvent({ ...base, status: "rate-limited",
        inputSummary: summarize(context), errorMessage: "global hourly quota" }).catch(() => {});
      return { ok: false, implemented: true, rateLimited: true, feature, error: "AI request quota exceeded" };
    }
