...
export function bridgeStatus() {
  return {
    ...
    live: [
      ...
      "canonical event history (lib/events → user_events via lib/db; interaction, board saves, image uploads write it)",
      "palette v0 (lib/vision/palette.js): moodboard uploads read real pixel color statistics → tag-space training (colors only, honestly labeled)",
      "cross-user layer (lib/taste-graph + /api/similar): taste neighbors via profile-cosine scan, collaborative candidates scored from event history — compute-on-read, prototype scale",
    ...
    placeholder: [
      "deep image analysis (objects/texture/silhouette) + fit-pic analyzer (lib/vision, provider-gated)",
      "taste clusters + precomputed user_similarity at scale (lib/taste-graph)",
      "source connections: Pinterest OAuth (Shopify merchant OAuth is implemented separately)",
      "background learning jobs (the catalog queue is real; learning jobs remain unwired)",
