...
export async function rebuildUserStyleProfile(userId) {
  ...
  try {
    const [signals, brainProfile, boards, feedback, corrections] = await Promise.all([
    ...
    const NEGATIVE_CODES = new Set(["not-my-style", "less-like-this", "dont-recommend-silhouette"]);
    for (const c of corrections) {
      if (NEGATIVE_CODES.has(c.code)) {
        for (const t of c.tags || []) avoided[t] = (avoided[t] || 0) + 2;
      } else if (c.code === "more-like-this") {
        for (const t of c.tags || []) {
          aesthetics[t.toLowerCase()] = +((aesthetics[t.toLowerCase()] || 0) + 0.3).toFixed(3);
        }
      }
    }
    const avoidedBrands = [...new Set(
      corrections.filter((c) => c.code === "dont-recommend-brand" && c.brand)
