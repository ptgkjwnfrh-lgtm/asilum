...
/**
 * The AI feature configuration, read fresh from the environment each call.
 *
 * `hasKey` is a BOOLEAN — presence only. The key's value never appears in this
 * object, so the result is safe to pass around the server; use aiApiKey() at
 * the point of the call instead.
 *
 * Not memoized on purpose: a flag flipped in the environment takes effect
 * without a deploy.
 */
export function aiConfig() {
  return {
    ...
    features: {
      ...
      "tag-audit": truthy(process.env.AI_TAG_AUDIT_ENABLED),
      // AI_SEARCH_ENABLED was declared in .env.example and read by exactly one
      // file that nothing imported — an orphan flag pointing at dead code.
      // It is a real feature switch now, and like every other one it only
      // does anything with the master switch, a provider and a key present.
      search: truthy(process.env.AI_SEARCH_ENABLED),
      research: truthy(process.env.AI_RESEARCH_ENABLED),
    },
  };
}
