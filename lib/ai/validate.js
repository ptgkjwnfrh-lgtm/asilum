...
/**
 * Strip a model's prose down to something safe to render.
 *
 * Removes tags, collapses whitespace, truncates. MODEL OUTPUT IS UNTRUSTED
 * INPUT — it may echo a prompt-injected instruction or markup from a page the
 * model read — so it gets the same treatment as a client payload.
 *
 * Returns "" for a non-string rather than throwing, because a caller
 * discovering the model returned an object should degrade to local rules, not
 * crash the request.
 */
