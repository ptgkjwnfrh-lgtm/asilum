...
/**
 * An analysis result flattened into the house `{tag, tag_type, confidence}` rows.
 *
 * NOTE THE FACET NAMES HERE ARE THE MOOD-BOARD VOCABULARY, not the product one
 * — these rows describe a person's UPLOAD, and are stored on the mood-board
 * record. They never enter `product_tags`, which is fenced to the register in
...
export async function extractMoodBoardSignals(userId) {
  ...
  // analysis_source is written at :70-73 and was dropped right here: the read
  // side filtered on analysisStatus alone, so a tag the MODEL proposed and a
  // tag the local rules derived became the same undifferentiated weight. That
  // matters downstream — this map feeds lib/ai/styleProfile.js, whose profile
  // lib/asterisk/queryRouter.js reads to order which cultural reading of a
  // search query leads. Counting them separately costs nothing and makes the
  // profile able to say where its taste came from.
  let modelAnalyses = 0;
  for (const a of analyses) {
    if (a.analysisStatus !== "complete") continue;
    if (a.analysisSource === "model") modelAnalyses++;
    for (const t of a.parsedTags || []) add(t.tag, t.tag_type, (t.confidence ?? 0.5) * 0.8);
  }
  return { byType, uploads: uploads.length, analyses: analyses.length, modelAnalyses };
}
