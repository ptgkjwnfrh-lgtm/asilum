...
// Day 27 — SEARCH INTERPRETATION. The model is a PARSER here, never an
// oracle: it turns a sentence into the engine's OWN constraint language and
// nothing else. Every list it may draw from is handed to it closed, and
// lib/ai/validate.js drops anything outside those lists, so the worst a
// mis-parse can produce is a constraint the deterministic engine could have
// produced itself. It is never asked for a product, a house it was not given,
// a fact about a garment, or a confidence in its own work.
export const PROMPT_VERSION_SEARCH_INTERPRETATION_V1 = {
  id: "search-interpretation-v1",
  feature: "search",
  template: `You are a PARSER for ASILUM's fashion search. Turn the shopper's
sentence into the search engine's own constraint language. You are not a
stylist and not an encyclopedia: do not name products, do not name houses that
are not in the list below, and do not state facts about garments.

QUERY: {{query}}

You may use ONLY these values. Anything else you return is discarded.
garment nouns: {{garments}}
aesthetic axes: {{aesthetics}}
origins: {{origins}}
size letters: {{sizes}}

Respond with ONLY this JSON, no other text:
{
  "garments": [], "aesthetics": [], "origins": [], "size": null,
  "era": { "minYear": null, "maxYear": null },
...
Rules:
- garments, aesthetics, origins, size: values copied EXACTLY from the lists.
- era: production years, 1900..2100, or nulls. "vintage" is 20+ years old.
- minPrice/maxPrice: whole US dollars, or null. Never invent a budget the
  sentence does not state.
- exclusions: single words the shopper asked NOT to see. Max 3.
- reading: one short phrase, at most 12 words, describing what you took the
  sentence to mean. No claims about specific pieces.
- Leave a field null or empty when the sentence does not support it. An empty
  answer is correct and useful; a guess is not.`,
};

export const PROMPTS = {
  [PROMPT_VERSION_SEARCH_INTERPRETATION_V1.id]: PROMPT_VERSION_SEARCH_INTERPRETATION_V1,
