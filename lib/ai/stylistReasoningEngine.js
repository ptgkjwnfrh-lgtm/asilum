...
import { applyRecommendationExclusions, getDiscoverablePool } from "../products.js";

/** The comparable size string for an item.
 *
 *  Catalog items carry `size` as an OBJECT
 *  ({label, system, gender, fitsLikeUS, runsBias, measurements}), so any
 *  String(it.size) is "[object Object]". `fitsLikeUS` is the brand-adjusted US
 *  equivalent, which is what a user's "usual size" actually means — a Prada M
 *  fitsLikeUS "S", and warning that against a user who wears S would be the
 *  wrong way round. Falls back to the garment's own label, and still accepts a
 *  plain string so a caller supplying one keeps working. */
export function sizeLabelOf(item) {
  const s = item?.size;
  if (s == null) return null;
  if (typeof s === "string") return s.trim() || null;
  if (typeof s !== "object") return null;
  return s.fitsLikeUS || s.label || null;
}
...
export async function getCandidateProductsForStylist(request = {}, userId = null) {
  ...
  const exclusions = userId
    ...
    : { brands: [], productIds: [] };
  return applyRecommendationExclusions(pool, exclusions).filter((it) => {
...
function reasonings(built, profile, request) {
  ...
  return {
    ...
    trendLogic: trendMatches.length
      ...
      : null,
    warnings: [
      // WHAT WAS WRONG (Aug 8, codebase audit). This compared
      // `String(it.size)` against the user's size preferences. Every one of
      // the 915 catalog items carries `size` as an OBJECT —
      // {label, system, gender, fitsLikeUS, runsBias, measurements} — so
      // String(it.size) was the literal "[object Object]", which is never in
      // sizePreferences. EVERY sized piece in EVERY look was warned "size may
      // not match preference", including the ones that matched exactly.
      //
      // Not a rare path: app/api/outfits passes sizePreferences from the
      // user's own fit profile, so anyone who set their size got a wall of
      // warnings that were all false. The list is .slice(0,4)'d, so real
      // availability warnings were pushed out by the noise.
      //
      // Compared against fitsLikeUS — the brand-adjusted US equivalent, which
      // is what a user's "usual size" means — falling back to the garment's
      // own label, and still accepting a plain string for any caller that
      // supplies one.
      ...(request.sizePreferences?.length
        ? items.filter((it) => {
          const s = sizeLabelOf(it);
          return s && !request.sizePreferences.includes(s);
        }).map((it) => `${it.title || it.id}: size may not match preference`)
        : []),
...
/** Order whole outfits by how well their pieces suit a reader — the SUM of
 *  per-piece taste scores plus the outfit's own coherence score, so a look is
 *  judged as an assembly rather than by its best single garment. */
...
/** Persist a stylist request. Thin pass-through, kept so callers depend on
 *  this engine rather than reaching into lib/db directly. */
export async function saveStylistRequest(request) { return createStylistRequest(request); }
/** Persist a reader's verdict on a look. Feeds correction signals, which is
 *  why it is recorded even when the reader simply moves on. */
