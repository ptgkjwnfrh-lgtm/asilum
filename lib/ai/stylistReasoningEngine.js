...
import {
  ...
  createStylistFeedback, getStylistOutfit, listUserCorrections,
...
export async function getCandidateProductsForStylist(request = {}, userId = null) {
  ...
  if (userId) {
    const corrections = await listUserCorrections(userId, 200).catch(() => []);
    excludedBrands = new Set(corrections
      .filter((c) => c.code === "dont-recommend-brand" && c.brand)
      .map((c) => c.brand.toLowerCase()));
  }
  return pool.filter((it) => {
    if (excludedBrands.size && it.brand && excludedBrands.has(it.brand.toLowerCase())) return false;
