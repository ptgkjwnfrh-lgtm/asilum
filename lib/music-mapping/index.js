...
/** A music genre to its tag vector, from a CURATED seed table.
 *  Returns notImplemented for anything not in the table — never a guess, and
 *  hits are mock-marked so a caller can tell a seeded reading from a sourced
 *  one. */
...
/** An artist to their style influence, from the same curated table and with
 *  the same honesty: unknown returns notImplemented rather than an inference. */
