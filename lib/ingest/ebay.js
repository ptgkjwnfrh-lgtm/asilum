...
export function guessCategory(title = "") {
  ...
  // shoes a seller names without the word shoe: the ghost run (10 Sep) filed
  // Ferragamo pumps, kitten heels and cuban-heel boots under tops
  if (/\b(pumps?|heels?|slingbacks?|stilettos?|court shoes?|flats|sandals?|clogs?|brogues?|espadrilles?|platforms?|wedges?)\b/.test(t)) return "footwear";
  // "dress shirt" is a shirt (the dresses rule below would take it)
  if (/\bdress shirts?\b/.test(t)) return "tops";
