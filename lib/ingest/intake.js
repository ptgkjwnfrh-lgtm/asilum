...
export function validateIntakeBatch(rawItems, sourceName) {
  ...
  if (!/^[a-z0-9-]{2,40}$/.test(source)) {
    return { ok: false, problems: [{ index: null, reason: "sourceName must be 2-40 chars of a-z, 0-9, hyphen" }], normalized: [] };
  }
  if (FORBIDDEN_SOURCES.test(source)) {
    return { ok: false, problems: [{ index: null, reason: `sourceName "${source}" reads as demo/test — real inventory only` }], normalized: [] };
  }
