...
export function typedTagsFrom(p) {
  ...
  // "Unknown" is what the normalizer writes when no source named a house. It
  // is an honest value in the items row and a LIE in the tag register, where a
  // brand facet means "this piece is by this house" — and where search would
  // then offer `unknown` as a brand anyone could filter by.
  if (p.brand && !/^unknown$/i.test(String(p.brand).trim())) add(p.brand, "brand", 0.9);
  ...
  for (const [t, v] of Object.entries(p.tags || {})) {
    if (TAGS.includes(t) && v >= 0.3) add(t, "aesthetic", Math.min(0.8, v));
  }
  // The stream's typed descriptors (lib/asterisk/stream/tags.js composeTags)
  // arrive already faceted; they go through the same vocabulary gate.
  for (const row of Array.isArray(p.typedTags) ? p.typedTags : []) {
    if (!row?.tag) continue;
    const tagType = facetOf(row.tagType);
    if (!tagType) continue;
    out.push({ tag: String(row.tag).toLowerCase(), tagType, confidence: Math.max(0, Math.min(1, Number(row.confidence) || 0.5)), source: row.source || "system" });
  }
  return out;
}
...
export function normalizeSourceProduct(raw, sourceName) {
  ...
  // TWO LAYERS IN ONE OBJECT (lib/asterisk/stream/tags.js): the ten canonical
  // aesthetics, UPPERCASE; and the stream's descriptors, lowercase, capped.
  // Anything else a source claims as a tag is dropped here.
  const tags = {};
  let descriptorCount = 0;
  for (const [tag, value] of Object.entries(inferred || {})) {
    const v = Number(value);
    if (!Number.isFinite(v) || v <= 0) continue;
    const upper = String(tag).toUpperCase();
    if (TAGS.includes(upper)) { tags[upper] = Math.min(1, v); continue; }
    const lower = String(tag);
    if (lower === lower.toLowerCase() && DESCRIPTOR_RE.test(lower) && descriptorCount < DESCRIPTOR_CAP) {
      tags[lower] = Math.min(0.9, v); descriptorCount += 1;
    }
  }
  ...
  const normalizedSize = suppliedSize?.label && !suppliedSize.fitsLikeUS
    ? { ...sizeRecord(suppliedSize.label, {
        ...
        measurementSource: suppliedSize.measurementSource,
      }), ...suppliedSize, category }
    : suppliedSize;
  return {
    id,
    source_name: source,
    ...
    source,
    // the stream's records ride through unchanged: the typed descriptor rows
    // for product_tags and the identification for asterisk_identifications
    ...(Array.isArray(raw.typedTags) ? { typedTags: raw.typedTags } : {}),
    ...(Array.isArray(raw.descriptors) ? { descriptors: raw.descriptors } : {}),
    ...(raw.identification && typeof raw.identification === "object" ? { identification: raw.identification } : {}),
  };
}
