...
import { processQuery } from "../../asterisk/stream/index.js";
import { normalizeSourceProduct } from "./normalize.js";
import { readJsonResponse } from "../../security/http.js";
import { verifyProductColor } from "../colorEvidence.js";
...
export const ebayAdapter = {
  ...
  async searchProducts(query, filters = {}) {
    ...
    const raw = await searchEbay(query, { limit: filters.limit || 24, verifyColors: false });
  ...
  async fetchProductById(sourceProductId) {
    ...
    return this.normalizeSourceProduct(await verifyProductColor(normalizeEbayItem(await readJsonResponse(res))));
  },
  ...
  // THE STREAM (lib/asterisk/stream): every synced listing is read in full —
  // the item detail with the seller's tags, the identification when the
  // model is keyed, the composed tags — before it is normalized. `deep`
  // follows identifyEnabled() unless the caller says otherwise.
  async syncProducts(opts = {}) {
    if (!ready()) return { sourceName: SOURCE, enabled: false, itemsSeen: 0, itemsUpserted: 0, status: "disabled" };
    const results = await processQuery(opts.query || "designer archive fashion", {
      limit: opts.limit || 48, concurrency: opts.concurrency || 3, deep: opts.deep,
    });
    const products = results.map((r) => this.normalizeSourceProduct(r.product));
    const identified = results.filter((r) => r.summary.identified).length;
    const costUsd = +results.reduce((s, r) => s + (r.summary.costUsd || 0), 0).toFixed(4);
    return { sourceName: SOURCE, enabled: true, itemsSeen: products.length, itemsUpserted: 0, status: "fetched", products, identified, costUsd, results };
  },
};
