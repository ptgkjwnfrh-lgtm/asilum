...
export const ebayAdapter = {
  ...
  enabled: () =>
    keys()
      ...
      : { enabled: false, status: "awaiting_keys", needs: "EBAY_CLIENT_ID + EBAY_CLIENT_SECRET (developer.ebay.com, Browse API)" },
  ...
  async fetchProductById(sourceProductId) {
    ...
    return this.normalizeSourceProduct(normalizeEbayItem(await res.json()));
  },

  async checkAvailability(sourceProductId) {
    if (!keys()) return { availability_status: "source_unavailable" };
    const env = process.env.EBAY_ENV === "PRODUCTION" ? "PRODUCTION" : "SANDBOX";
    const host = env === "PRODUCTION" ? "https://api.ebay.com" : "https://api.sandbox.ebay.com";
    try {
      const token = await getAppToken();
  ...
  normalizeSourceProduct(raw) {
    ...
    return normalizeSourceProduct(
      { ...raw, source_product_id: (raw.id || "").replace(/^ebay-/, ""), source_product_url: raw.url },
      SOURCE
    );
  },

  async syncProducts(opts = {}) {
    if (!keys()) return { sourceName: SOURCE, enabled: false, itemsSeen: 0, itemsUpserted: 0, status: "disabled" };
