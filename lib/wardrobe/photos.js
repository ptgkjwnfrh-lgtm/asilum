...
/** Is private object storage configured? Photos are refused without it rather
 *  than falling back to any public bucket. */
export function storageConfigured() {
  return !!storageEnv();
}

// The single availability gate the route and the wardrobe GET expose.
/** THE single availability gate, returning `{ available, reason }` so the
 *  route and the wardrobe GET give a reader the SAME sentence about why photos
 *  are off — one source, so the two cannot drift. */
...
/**
 * The storage path for one photo. THROWS on any component that fails its
 * pattern.
 *
 * The patterns are a path-traversal guard, not tidiness: these values become a
 * storage key, and an unvalidated id could reach another user's objects. The
 * version segment makes each write a new path, so a replaced photo cannot be
 * served from cache.
 */
...
/** Does this actually begin with the JPEG magic bytes? Checked SERVER-SIDE
 *  even though the client re-encodes — a content type is a claim, and the
 *  re-encode is also what strips EXIF (location) from a personal photo. */
...
/**
 * Store one wardrobe photo in PRIVATE object storage, returning its path.
 *
 * Re-checks the JPEG magic bytes and the size limit server-side even though
 * the client already did — a content type is a claim. `x-upsert: false` means
 * a write never silently replaces an existing object.
 *
 * These are a person's own clothes, photographed at home. They are private,
 * never part of the public catalog, and reachable only through a short-lived
 * signed URL.
 */
...
/** A short-lived signed URL for a private photo, or null if storage is
 *  unconfigured. THE ONLY WAY A PHOTO IS EVER SERVED — the bucket is private,
 *  and the expiry is what stops a shared link outliving the share. */
...
/** Delete one photo object. Part of retiring an item and of the §6 privacy
 *  delete — a wardrobe row removed without its photo would leave the picture
 *  behind. */
