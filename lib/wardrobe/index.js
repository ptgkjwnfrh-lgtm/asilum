...
/** Feature flag: may a reader attach a private photo to a wardrobe item?
 *  OFF unless `WARDROBE_UPLOADS_ENABLED=1` — deliberately opt-IN, and separate
 *  from `WARDROBE_ENABLED`, so the wardrobe runs with no photo storage at all. */
...
export async function normalizeWardrobeAdd(userId, input = {}) {
  ...
  if (source === "ticket") {
    ...
    // purchase_tickets.id is BIGSERIAL. A non-numeric id was only ever
    // string-checked, so mem answered a clean 400 ("ticket not found",
    // via Number("abc") → NaN) while Postgres raised 22P02 through an
    // unguarded query and the route returned 500 (audit resume, Aug 14 —
    // law 4, a mem/Postgres divergence that ships green because mem is
    // what runs in dev, preview, and 520 of 532 tests). Refusing the
    // shape here makes both backends answer identically.
    if (!/^\d{1,18}$/.test(ticketId)) return { ok: false, error: "ticket not found" };
...
/** Add a piece to a reader's wardrobe. Returns `{ ok, item, duplicate }` —
 *  `duplicate: true` means it was already there and nothing was written, which
...
/** A reader's wardrobe. Defaults to `active` — retired pieces stay on record
 *  (they are part of what the stylist learned from) but are out of the way
 *  until asked for. */
