...
// Repeat queries are the norm in search — memo successful single-text embeds
// for the process lifetime (bounded; oldest entry evicted). Failures are
// never cached, so a rate-limited call retries clean next time.
...
export async function embedText(text) {
  ...
  const memoKey = `${info.name}/${info.model}:${text}`;
  const cached = embedMemo.get(memoKey);
  if (cached) return cached;
  try {
    const [vector] = await providerEmbed([text]);
    const out = real({ vector, provider: info.name, model: info.model, space: TEXT_SPACE },
      `${info.name}/${info.model} text embedding`);
    if (embedMemo.size >= EMBED_MEMO_MAX) embedMemo.delete(embedMemo.keys().next().value);
    embedMemo.set(memoKey, out);
    return out;
  } catch (e) {
