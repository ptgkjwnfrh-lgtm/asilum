...
export const EVENTS = Object.freeze({
  ...
  USER_COMPLETED_PURCHASE: "USER_COMPLETED_PURCHASE",     // live (19 Sep) → a sale order settled paid teaches the brain (lib/brain/purchase.js)
...
const ACTION_TO_EVENT = {
  ...
  open: EVENTS.USER_VIEWED_PRODUCT,  // (7 Sep) an explicit open is a view the reader chose
...
const BRIDGE_ATTRIBUTIONS = new Set([
  ...
  // (6 Sep) the catalog lane. Without it here the server's own stamp for a
  // lane slot never outranked the client's, and a client could relabel a
  // lane engagement as any tunable bridge (a verifier's finding). Tuning
  // sums only its TUNABLE keys, so the stored value stays inert.
  "catalog",
]);

// (r24) Serve context zones. Same whitelist discipline as the bridges above:
// an unknown value is dropped rather than stored.
const SERVE_ZONE_SET = new Set(["core", "discovery", "reach", "catalog"]);
const MAX_SLOT = 999;

/**
 * @param serveContext  The SERVER's own record of where this item sat when it
 *   was served — `serveContextFor(profile, itemId)` from lib/brain. It is a
 *   separate argument from `item` on purpose: `item` has passed through the
 *   client, and position is exactly the field a client would have reason to
 *   misreport. Nothing on `item` can supply slot or zone, whatever it carries.
 */
export function eventFromInteraction(userId, action, item, dwellMs = null, serveContext = null) {
  ...
  // AD FIREWALL (audit #19): tag sponsored engagement so the cross-user
  // discovery reader (lib/taste-graph) can exclude it — a sponsored item must
  // not propagate into other users' feeds through the event graph either.
  if (item?.sponsored === true) payload.sponsored = true;
  ...
  // (r24) position context, server-stamped. Absent — not zero, not guessed —
  // when this interaction did not come from the last recorded serve.
  if (serveContext) {
    const slot = serveContext.slot;
    if (Number.isInteger(slot) && slot >= 0 && slot <= MAX_SLOT) payload.slot = slot;
    if (SERVE_ZONE_SET.has(serveContext.zone)) payload.zone = serveContext.zone;
    // A server-recorded bridge outranks the client-reported one when both
    // exist: same fact, better source.
    if (BRIDGE_ATTRIBUTIONS.has(serveContext.bridge)) payload.bridge = serveContext.bridge;
  }
